...
\end{multline*}
where the integral with respect to $\{Y_s\}_s$ is a \emph{Wick It\^o Skorohod (WIS)} integral (see \cite[Chapter 4]{BHOB2008}), provided all the integrands are integrable in a suitable sense with respect to $\{Y_t\}$.

In turn, for estimating the integral with respect to $\{Y_s\}_s$, we need a version of the It\^o isometry. A general one is the \emph{WIS isometry} in \cite[Theorem 4.5.6]{BHOB2008}, which is, however, a delicate formula involving fractional derivatives. 
...
\begin{lem}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            f(t, Y_t) & = f(0, Y_0) + \int_{0}^t \frac{\partial f}{\partial s}(s, B_{s, t})\;\mathrm{d}s + \alpha \int_{0}^t \frac{\partial f}{\partial y}(s, B_{s, t})s^{\alpha - 1} (Z_{s, t} - Z_{s, s})\;\mathrm{d}s \\
    ...
    \begin{equation}
        \label{defYHst}
        Z_{s, t} = C_H \int_0^s \tau^{-\alpha}(t - \tau)^\alpha \;\mathrm{d}W_\tau
    \end{equation}
    and
    \begin{equation}
        \label{defBHst}
        Y_{s, t} = s^\alpha Z_{s, t} - \alpha \int_0^s \zeta^{\alpha-1}Z_{\zeta, \zeta}\;\mathrm{d}\zeta.
    \end{equation}
...
\[
    m_H(t, s) = C_H \left( \left(\frac{t}{s}\right)^\alpha (t - s)^\alpha - \alpha s^{-\alpha}\int_s^t\zeta^{\alpha-1}(\zeta - s)^\alpha\;\mathrm{d}\zeta\right),
\]
and where $C_H$ is given by \eqref{defCH}. The constant $C_H$ is the constant $C_H^{(6)}$ in \cite[Section 1.8]{Mishura2008}.

Thus, as in \cite[Section 2.7.4]{Mishura2008}, we define $Z_{s, t}$ and $Y_{t, s}$, for $0 \leq s \leq t$, as in \eqref{defYHst} and \eqref{defBHst}, and notice that $Y_t = Y_{t, t}$ and $Y_0 = Y_{0, t} = 0$. As functions of $s$, both $Z_{s, t}$ and $Y_{s, t}$ are non-anticipative with respect to the Wiener process and are It\^o processes, with
\begin{equation}
...
\[
    \mathbb{E}\left[(Z_{s, t})^2\right] = C_H^2 \int_0^s \tau^{-2\alpha}(t - \zeta)^{2\alpha}\;\mathrm{d}\zeta \\
    \leq C_H^2 s^{-2\alpha}t^{2\alpha}s = C_H^2 s^{1-2\alpha}t^{2\alpha}.
\]
Hence, $\mathbb{E}[|Z_{s, t}|] = \mathcal{O}(s^{1/2-\alpha})$, as $s\rightarrow 0$, and the singularities at both endpoints $s=0$ and $s=t$ are all integrable.

Now, the formula \eqref{partialItoformula} follows from the classic It\^o formula using that $s \mapsto B_{s, t}$ is an It\^o process satisfying \eqref{BHstItoprocess}, together with $Y_{t, t} = Y_t$ and $Y_{0, t} = 0 = Y_0$.
\end{proof}
...
\begin{lem}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            & f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \\
            & \quad = \int_{\tau^N(s)}^{s} G(\zeta, s) \;\mathrm{d}\zeta + \int_0^{\tau^N(s)} (G(\zeta, s) - G(\zeta, \tau^N(s))) \;\mathrm{d}\zeta \\
            & \quad + \int_{\tau^N(s)}^{s} H(\zeta, s) \;\mathrm{d}W_\zeta + \int_0^{\tau^N(s)} (H(\zeta, s) - H(\zeta, \tau^N(s))) \;\mathrm{d}W_\zeta,
        \end{aligned}
    \end{equation}
    for the families of processes $\{G(\tau, s)\}_{0\leq\tau\leq s}$ and $\{H(\tau, s)\}_{0\leq\tau\leq s}$, $0\leq s \leq T$, given by
    \begin{multline}
        G(\tau, s) = \frac{\partial f}{\partial \tau}(\tau, X_{\tau^N(s)}^N, Y_{\tau, s})  + \alpha \frac{\partial f}{\partial y}(\tau, X_{\tau^N(s)}^N, Y_{\tau, s})\tau^{\alpha - 1} (Z_{\tau, s} - Z_{\tau, \tau}) \\ 
        + \frac{C_H^2}{2}\frac{\partial^2 f}{\partial y^2}(\tau, X_{\tau^N(s)}^N, Y_{\tau, s}) (s - \tau)^{2\alpha}
    \end{multline}
    and
    \begin{equation}
        H(\tau, s) = C_H\frac{\partial f}{\partial y}(\tau, X_{\tau^N(s)}^N, Y_{\tau, s})(s - \tau)^\alpha.
    \end{equation}
\end{lem}
...
\begin{proof}
    ...
    \begin{align*}
        f(s, Y_s) & = f(0, Y_0) + \int_{0}^s \frac{\partial f}{\partial \zeta}(\zeta, X_{\tau^N(s)}^N, B_{\zeta, s})\;\mathrm{d}\zeta \\ 
        & \quad + \alpha \int_{0}^s \frac{\partial f}{\partial y}(\zeta, X_{\tau^N(s)}^N, B_{\zeta, s})\zeta^{\alpha - 1} (Z_{\zeta, s} - Z_{\zeta, \zeta})\;\mathrm{d}\zeta \\
        & \quad + \frac{C_H^2}{2}\int_{0}^s \frac{\partial^2 f}{\partial y^2}(\zeta, X_{\tau^N(s)}^N, B_{\zeta, s}) (s - \zeta)^{2\alpha} \;\mathrm{d}\zeta \\
        & \quad + C_H \int_{0}^s \frac{\partial f}{\partial y}(\zeta, X_{\tau^N(s)}^N, B_{\zeta, s})(s - \zeta)^\alpha\;\mathrm{d} W_\zeta.
    \end{align*}
    In terms of $G$ and $H$, this becomes
    \[
        f(s, Y_s) = f(0, Y_0) + \int_0^s G(\zeta, s)\;\mathrm{d}\zeta + \int_0^s H(\zeta, s)\;\mathrm{d}W_{\zeta}.
    \]
...
\begin{proof}
    From \eqref{singlestepwithfBm},
    \begin{align*}
        & \int_0^t \left(f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})\right)\;\mathrm{d}s \\
        & \quad = \int_0^t\int_{\tau^N(s)}^{s} G(\zeta, s) \;\mathrm{d}\zeta\;\mathrm{d}s + \int_0^t\int_0^{\tau^N(s)} (G(\zeta, s) - G(\zeta, \tau^N(s))) \;\mathrm{d}\zeta\;\mathrm{d}s \\
        & \quad + \int_0^t\int_{\tau^N(s)}^{s} H(\zeta, s) \;\mathrm{d}W_\zeta\;\mathrm{d}s + \int_0^t\int_0^{\tau^N(s)} (H(\zeta, s) - H(\zeta, \tau^N(s))) \;\mathrm{d}W_\zeta\;\mathrm{d}s.
    \end{align*}
    Using Fubini's Theorem,
    \begin{align*}
        & \int_0^t \left(f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})\right)\;\mathrm{d}s \\
        & \quad = \int_0^t\int_\zeta^{\tau^N(\zeta) + \Delta t_N} G(\zeta, s) \;\mathrm{d}s\;\mathrm{d}\zeta + \int_0^t\int_{\tau^N(\zeta) + \Delta t_N}^t (G(\zeta, s) - G(\zeta, \tau^N(s))) \;\mathrm{d}s\;\mathrm{d}\zeta \\
        & \quad + \int_0^t\int_\zeta^{\tau^N(\zeta) + \Delta t_N} H(\zeta, s) \;\mathrm{d}s\;\mathrm{d}W_\zeta + \int_0^t\int_{\tau^N(\zeta) + \Delta t_N}^t (H(\zeta, s) - H(\zeta, \tau^N(s))) \;\mathrm{d}s\;\mathrm{d}W_\zeta.
    \end{align*}
    Now we need to estimate each term.
\end{proof}
