...
Under certain regularity conditions on $f$, it is proved in \cite{GruneKloeden2001} that, when the noise $\{Y_t\}_{t\in I}$ has $\theta$-H\"older continuous sample paths, the Euler scheme converges \emph{pathwise} with order $\theta$ with respect to the time step. Under global uniformity conditions on the nonlinear term, the same approach in \cite{GruneKloeden2001} yields the convergence of order $\theta$ also in the \emph{strong} sense \eqref{strongordertheta}.
...
The global uniformity conditions on the nonlinear term are natural assumption when looking for \emph{strong convergence} for random ordinary differential equations, as well as stochastic differential equations. \emph{Pathwise convergence,} on the other hand, usually requires less stringent conditions, in the sense of not being necessarily uniform (see e.g. \cite{JentzenKloedenNeuenkirch2009, JentzenKloeden2011}), but those are not the subject of interest here. It is not yet clear whether the same ideas here can be used to obtain higher order regularity for pathwise convergence. This will be investigated in a future oportunity.
...
Here, the noise $\{Y_t\}_{t\in I}$ is \emph{not} assumed to be an It\^o process noise, but, instead, that the steps can be controlled by monotonic nondecreasing processes with finite expected growth. This fits well the typical case of point processes, such as renewal-reward processes, Hawkes process, and the like.
...
\begin{proof}
    ...
    \end{multline*}
    Since this holds for every $j=0, \ldots, N$, we obtain the desired \cref{thmmonotonicboundstrongordernew}.
\end{proof}
...
\begin{rmk}
    ...
    \]
    Since the partial derivatives of $a=a(t, y)$ and $b=b(t, y)$ are uniformly bounded and $h$ is Lipschitz, it follows that the partial derivatives $\partial_t f$ and $\partial_y f$ have at most linear growth. Thus, \eqref{ftfylineargrowth} is satisfied and \cref{thmdiffmonotonicbound} applies. Similarly for the vector-values cases. But this special form \eqref{lineareqform} is by no means necessary, and the result applies to more general terms $f=f(t, x, y)$, as stated in the theorem.
\end{rmk}
...
In the vector-valued case, $\{A_t\}_{t\in I}$ is a vector-valued process with the same dimension as $Z_t$, while $\{B_t\}_{t\in I}$ is a square-matrix-valued process with the number of rows and of columns matching the dimension of $Z_t.$ The (multi-dimensional) Wiener process $\{W_t\}_{t\geq 0}$ is a vector-valued process $W_t = (W_t^{(j)})_j$ with the dimension matching the number of columns of $B_t$ and where each coordinate $\{W_t^{(j)}\}_{t\in I}$ is a Wiener process independent of the processes in the other coordinates. Both $\{A_t\}_{t\in I}$ and $\{B_t\}_{t\in I}$ are assumed to be adapted to each $\{W_t^{(j)}\}_{t\in I}.$ Writing the \emph{columns} of $B_t$ as $B_t^{(j)},$ we can write the stochastic term in \eqref{Itoprocess} as $B_t\;\mathrm{d}W_t = \sum_i B_t^{(j)}\;\mathrm{d}W_t^{(j)}.$ More explicitly, writing in full coordinates $Z_t = (Z_t^{(i)})_i,$ $A_t = (A_t^{(i)})_i,$ and $B_t = (B_t^{(i, j)})_{i, j},$ we have \eqref{Itoprocess} as the system
\begin{equation}
...
\begin{thm}
    \label{thmItostep}
    Under \cref{standinghypotheses1}, suppose also that
    \eqref{EX0strongbound}, \eqref{EMtstrongbound}, \eqref{EX0X0N}, \eqref{Itoprocess}, and \eqref{expectItostepterms} hold. Then, the Euler scheme \eqref{emscheme}-\eqref{iccondition} is of strong order 1, i.e.
    \begin{equation}
...
\begin{proof}
    ...
    Thanks to \eqref{EX0strongbound}, \eqref{EMtstrongbound}, and \eqref{EX0X0N}, \cref{propbasicestimate} applies and the global error is bounded according to \eqref{expectedestimateglobalerrorintegral}.
    ...
    \end{multline*}
    Since this holds for every $j=0, \ldots, N$, we obtain the desired \cref{thmItosteptrongordernew}.
\end{proof}

In practice, conditions \eqref{Itoprocess}-\eqref{expectItostepterms} follows from assuming sufficent regularity on $f=f(t, x, y)$ and an It\^o process noise. We assume the noise satisfies
\begin{equation}
    \label{YtItonoise}
...
\begin{proof}
    ...
    Using the It\^o formula with \eqref{YtItonoise} in mind (see \cite[Theorem 4.1.2]{Oksendal2003} or \cite[Section 7.4]{Kuo2006} for the scalar case of the It\^o formula, with $g(t, y) = y^2,$ and \cite[Theorem 4.2.1]{Oksendal2003} or \cite[Section 7.5]{Kuo2006} for the multi-dimensional case, with $g(t, y) = \|y\|^2$), we have
    \[
    ...
    Now, in view of \cref{thmItostep}, it remains to prove that $F_t^N = f(t, X_{\tau^N(t)}^N, Y_t)$ is an It\^o process \eqref{Itoprocess}, with the bounds \eqref{expectItostepterms}. The fact that it is an It\^o process follows from the It\^o formula and the smoothness of $f=f(t, x, y)$. We first consider the case of a scalar noise, for the sake of clarity. Since $(t, y) \mapsto f(t, x, y)$ is twice continuously differentiable, for each fixed $x$, the It\^o formula is applicable and yields, in the case of a scalar noise,
    \begin{multline}
        \label{itoformula}
    ...
    In the case of a multi-dimensional noise, we have $a(t, y) = (a_i(t, y))_i$, $b(t, y) = (b_{ij}(t, y)),$ and \eqref{itoformula} becomes
    \begin{multline}
...
\end{equation}
As for the distribution of the integral over a mesh interval $[t_i, t_{i+1}]$ when given the endpoints $W_{t_i}$ and $W_{t_{i+1}},$ we have, similarly,
\begin{align*}
