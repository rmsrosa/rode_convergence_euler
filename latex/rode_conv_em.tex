...
\begin{lemma}
    ...
    \qed
\end{lemma}
The above estimate is sufficient for the pathwise convergence of the Euler method in the case of an FV noise. For the strong convergence, we need a stronger condition on $\{L_t\}_{t\in I}$, such as in the following result.
\begin{lemma}
    \label{lemXNtmeanboundLXMt}
    Under the \cref{standinghypotheses1}, assume moreover that \eqref{LtLXbound} and \eqref{esssuponMt} hold. Then, the Euler approximation \eqref{emscheme} for a pathwise solution of the random ordinary differential equation \eqref{rodeeq} satisfies
    \begin{equation}
...
\begin{lemma}
    ...
    Under the \cref{standinghypotheses1} and assuming moreover that $\{L_t\}_{t\in I}$ is essentially bounded on $I$, almost surely on $\Omega,$ as in \eqref{esssuponLt}, we find that
    \begin{equation}
...
\begin{theorem}
    ...
    Under the \cref{standinghypotheses1}, suppose further that \eqref{EX0strongbound}, \eqref{EMtstrongbound}, \eqref{LtLXbound} and \eqref{EX0X0N} hold, and that $f=f(t, x, y)$ is uniformly globally Lipschitz continuous in $x$ and is twice continuously differentiable in $(t, y)$, with partial differentials $\partial_t f$, $D_y f$ and $D_{yy} f$ satisfying \eqref{ftfylineargrowthcadlagfv} and
    \begin{equation}
