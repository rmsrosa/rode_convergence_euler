...
\begin{proof}
    ...
    For the second term, we use formula \cite[Theorem 29]{Protter2005}, which says that, for an adapted c\`adl\`ag process $\{H_t\}_{t\in I}$,
    \begin{equation}
        \left(\int_0^{t_j} H_\xi \;\mathrm{d}\xi\right)^2 = \int_0^{t_j} H_\xi^2 \;\mathrm{d}[X, X]_\xi.
    \end{equation}
    Hence,
    \begin{align*}
        & \mathbb{E}\left[\left\|\int_0^{t_j} \int_{\tau^N(s)^+}^s D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-}) \;\mathrm{d}X_\xi\;\mathrm{d}s\right\|\right] \\
        & \quad = \mathbb{E}\left[\left\|\int_0^{t_j} \int_{\xi}^{\tau^N(\xi) + \Delta t} D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-}) \;\mathrm{d}s \;\mathrm{d}X_\xi\right\|\right] \\
        & \quad \leq \mathbb{E}\left[\left(\int_0^{t_j} \int_{\xi}^{\tau^N(\xi) + \Delta t} D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-}) \;\mathrm{d}s \;\mathrm{d}X_\xi\right)^2\right]^{1/2} \\
        & \quad \leq \mathbb{E}\left[\int_0^{t_j} \left(\int_{\xi}^{\tau^N(\xi) + \Delta t} \|D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\| \;\mathrm{d}s\right)^2 \;\mathrm{d}[X, X]_\xi\right]^{1/2} \\
        & \quad \leq \Delta t\mathbb{E}\left[\int_0^{t_j} \|D_y f(\xi, X_{\tau^N(\xi)}^N, Y_{\xi^-})\|^2 \;\mathrm{d}[X, X]_\xi\right]^{1/2}
    \end{align*}
