...
Here we derive an estimate that is the basis for the specific estimates for each type of noise. For that, we use the following discrete version of the Grownwall Lemma, which is a particular case of the result found in \cite{GiraultRaviart1981} (see also \cite{Clark1987}): Let $(e_j)_j$ be a (finite or infinite) sequence of positive numbers satisfying
\begin{equation}
...
\begin{proof}
    ...
    \end{align*}
    This can be cast in the form of \eqref{integralgronwall}. Then, using the discrete version of the Gronwall Lemma, we obtain \eqref{estimateintegralgronwall}, which reads as \eqref{Etjbasicbound}.
\end{proof}
...
In this section, we illustrate the strong order 1 convergence with a few examples that fall into one of the cases considered above. We also illustrate the $H + 1/2$ order of convergence in the case of a fractional Brownian motion (fBm) noise with Hurst parameter $0 < H < 1/2$.

First we simulate a linear equation with several types of noises, emcompassing noises with sample paths with bounded variation and It\^o process noises. Next we simulate a linear equation with fBm noise. Then we implement a couple nonlinear problems. Further examples can be found in the github repository \cite{RODEConvEM2023}, such as a logistic model of population dynamics with random coefficients, loosely inspired by \cite[Section 15.2]{HanKloeden2017}, with an extra term representing harvest; and a Fisher-KPP partial differential equation \cite{Fisher1937} and \cite{KPP1937} with random boundary conditions, as inspired by the works of \cite{SalakoShen2020} and \cite{FreidlinWentzell1992}.
...
Table \ref{taballnoises} shows the estimated strong error obtained from 200 sample paths for each chosen time step, with initial condition $\mathbf{X}_0 \sim \mathcal{N}(\mathbf{0}, \mathrm{I})$, i.e. normally distributed on each coordinate, independently of the other coordinates, and on the time interval $[0.0, 1.0]$.
...
\cref{figallnoises} illustrates the order of convergence and 
sample paths of all the noises used in this system.
...
the excitation $\ddot M_t$ is made of a composition of oscillating signals $a_j t e^{-\delta_j t}\cos(\omega_j t + \theta_j)$ with random frequencies $\omega_j$, modulated by a linear attack rate $a_j t$ followed by an exponential decay $e^{-\delta_j t}$.
...
\begin{equation}
   ...
   \end{cases}
\end{equation}
on $t \geq 0$, with initial conditions $X_0$ and $Y_0$, where $\{A_t\}_{t\geq 0}$ and $\{B_t\}_{t\geq 0}$ are given stochastic process representing the external activation on each gene; $a$ and $c$ determine the auto-activation thresholds; $b$ and $d$ determine the thresholds for mutual repression; and $\mu$ and $\nu$ are protein decay rates. In this model, the external activations $A_t$ and $B_t$ are taken to be two independent process composed with a superposition of a compound Poisson process and a geometric Brownian motion process.

In the simulations below, we use parameters similar to those in \cite[Section 7.8]{Asai2016}. We fix $a = c = 0.25$; $b = d = 0.4$; and $\mu = \nu = 0.75$. The initial conditions are set to $X_0 = Y_0 = 4.0$. The external activations are superpositions of a compound Poisson process with Poisson rate $\lambda = 3.0$ and jumps uniformly distributed on $[0.0, 0.5]$ and a geometric Brownian motion process with initial value $0.5$, drift $0.2$ and diffusion $0.2$. The convergence estimate is done over the interval $[0, 5.0]$, while some illustrative solutions are over a longer interval $[0, 10.0]$.
...
For the mesh parameters, we set $N_{\textrm{tgt}} = 2^{18}$ and $N_i = 2^i$, for $i=4, \ldots, 9$. For the Monte-Carlo estimate of the strong error, we choose $m=1000.$ Table \ref{tabletoggleswitch} shows the estimated strong error obtained with this setup, while Figure \ref{figtoggleswitch} illustrates the order of convergence, a sample solution, the Euler approximations of a sample solution, and sample noises.
...
\begin{table}
    ...
    \caption{Mesh points (N), time steps (dt), and strong error (error) of the Euler method for a toggle-switch model of gene regulation for each N, with initial condition $X_0 = 4.0, Y_0 = 4.0$ and superposed gBm and compound Poisson process noises, on the time interval $(0.0, 5.0)$, based on $m = 1000$ sample paths for each fixed time step, with the target solution calculated with $262144$ points. The order of strong convergence is estimated to be $p = 0.98$}

    \label{tabletoggleswitch}
\end{table}

\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/toggleswitch_combined.png}
    \caption{(a) Order of convergence of the strong error of the Euler method for the toggle-switch model for gene expression \eqref{toogleswitchsystem}; (b) A sample solution $\{(X_t, Y_t)\}_t$; (c) Euler approximation of the component $X_t$ of a solution sample;  (d) Sample Compound Poisson (cP) and geometric Brownian motion (gBm) noise parts of $A_t$ and $B_t$.}
