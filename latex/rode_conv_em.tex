...
\begin{proof}
    ...
    \begin{align*}
        f(s, Y_s) & = f(0, Y_0) + \int_{0}^s \frac{\partial f}{\partial \xi}(\xi, X_{\tau^N(s)}^N, B_{\xi, s})\;\mathrm{d}\xi \\ 
        & \quad + \alpha \int_{0}^s \frac{\partial f}{\partial y}(\xi, X_{\tau^N(s)}^N, B_{\xi, s})\xi^{\alpha - 1} (Z_{\xi, s} - Z_{\xi, \xi})\;\mathrm{d}\xi \\
        & \quad + \frac{C_H^2}{2}\int_{0}^s \frac{\partial^2 f}{\partial y^2}(\xi, X_{\tau^N(s)}^N, B_{\xi, s}) (s - \xi)^{2\alpha} \;\mathrm{d}\xi \\
        & \quad + C_H \int_{0}^s \frac{\partial f}{\partial y}(\xi, X_{\tau^N(s)}^N, B_{\xi, s})(s - \xi)^\alpha\;\mathrm{d} W_\xi.
    \end{align*}
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            & \qquad + \left(\int_0^t(\tau^N(\xi) + \Delta t_N - \xi)\int_\xi^{\tau^N(\xi) + \Delta t_N} \mathbb{E}\left[\left|H(\xi, s)\right|^2\right] \;\mathrm{d}s\;\mathrm{d}\xi\right)^{1/2} \\
            & \qquad + \left(\int_0^t(t - \tau^N(\xi) - \Delta t_N)\right. \\
            & \qquad \qquad \qquad \qquad \left. \int_{\tau^N(\xi) + \Delta t_N}^t \mathbb{E}\left[\left|(H(\xi, s) - H(\xi, \tau^N(s)))\right|^2\right] \;\mathrm{d}s\;\mathrm{d}\xi\right)^{1/2}.
        \end{aligned}
    \end{equation}
    Now we need to estimate each integral. The integrals involving $H(\xi,s)$ are easier. Since the partial derivatives of $f$ are uniformly bounded, we have, from \eqref{defHtaus}, that
    \[
    ...
    \begin{multline*}
        \int_\xi^{\tau^N(\xi) + \Delta t_N} \mathbb{E}\left[\left|H(\xi, s)\right|^2\right] \;\mathrm{d}s \leq \int_\xi^{\tau^N(\xi) + \Delta t_N} C_H^2 L_Y^2 (s - \xi)^{2\alpha} \;\mathrm{d}s \\
        = \frac{C_H^2 L_Y^2}{2\alpha + 1} (\tau^N(\xi) + \Delta t_N - \xi)^{2\alpha + 1}.
    \end{multline*}
    Since $0 \leq \tau^N(\xi) + \Delta t_N \leq \Delta t_N$, we find that
    \begin{multline}
        \left(\int_0^t(\tau^N(\xi) + \Delta t_N - \xi)\int_\xi^{\tau^N(\xi) + \Delta t_N} \mathbb{E}\left[\left|H(\xi, s)\right|^2\right] \;\mathrm{d}s\;\mathrm{d}\xi\right)^{1/2} \\
        \leq t^{1/2}\frac{C_H L_Y}{(2\alpha + 1)^{1/2}} (\Delta t_N)^{\alpha + 1}.
    \end{multline}
    Since $\alpha = H - 1/2$, this term yields order $H + 1/2$.
