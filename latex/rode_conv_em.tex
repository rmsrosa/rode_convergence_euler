...
\begin{lem}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            f(t, B^H_t) & = f(0, B^H_0) + \int_{0}^t \frac{\partial f}{\partial s}(t, B_{s, t})\;\mathrm{d}s + \alpha \int_{0}^t \frac{\partial f}{\partial y}(s, B_{s, t})s^{\alpha - 1} (Y^H_{s, t} - Y^H_{s, s})\;\mathrm{d}s \\
            & \quad + \frac{C_H^2}{2}\int_{0}^t \frac{\partial^2 f}{\partial y^2}(s, B_{s, t}) (t - s)^{2\alpha} \;\mathrm{d}s + C_H \int_{0}^t \frac{\partial f}{\partial y}(s, B_{s, t})(t - s)^\alpha\;\mathrm{d} B_s,
        \end{aligned}
    \end{equation}
...
\]
where $C_H$ is given by \eqref{defCH}. The constant $C_H$ is the constant $C_H^{(6)}$ in \cite[Section 1.8]{Mishura2008}.

Thus, as in \cite[Section 2.7.4]{Mishura2008}, we define $Y^H_{s, t}$ and $B^H_{t, s}$, for $0 \leq s \leq t$, as in \eqref{defYHst} and \eqref{defBHst}, and write $B^H_t = B^H_{t, s}$. As functions of $s$, both $Y^H_{s, t}$ and $B^H_{s, t}$ are non-anticipative with respect to the Wiener process and are It\^o processes, with
\begin{equation}
...
\]
Hence, $\mathbb{E}[|Y^H_{s, t}|] = \mathcal{O}(s^{1/2-\alpha})$, as $s\rightarrow 0$, and the singularities at both endpoints $s=0$ and $s=t$ are all integrable.

Now, the formula \eqref{partialItoformula} follows from the classic It\^o formula using that $s \mapsto B_{s, t}$ is an It\^o process satisfying \eqref{BHstItoprocess}, together with $B^H_{t, t} = B^H_t$ and $B^H_{0, t} = 0 = B^H_0$. This completes the proof.
\end{proof}
...
\]
Since $X_{\tau^N(s)}^N$ is the same in both terms, we regard this step as involving a function of two arguments and apply \cref{lempartialItoformula} to find the following.
...
\begin{lem}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            & \quad = \int_{\tau^N(s)}^{s} H(\tau, s) \;\mathrm{d}B^H_\tau + \int_0^{\tau^N(s)} (H(\tau, s) - H(\tau, \tau^N(s))) \;\mathrm{d}B^H_\tau,
        \end{aligned}
    \end{equation}
    for the families of processes $\{G(\tau, s)\}_{0\leq\tau\leq s}$ and $\{H(\tau, s)\}_{0\leq\tau\leq s}$, $0\leq s \leq T$, given by
    \[
        G(\tau, s) = \frac{\partial f}{\partial \tau}(s, B^H_{\tau, s})  + \alpha \frac{\partial f}{\partial y}(\tau, B^H_{\tau, s})\tau^{\alpha - 1} (Y^H_{\tau, s} - Y^H_{\tau, \tau}) + \frac{C_H^2}{2}\frac{\partial^2 f}{\partial y^2}(\tau, B^H_{\tau, s}) (s - \tau)^{2\alpha}
    \]
    and
    \[
        H(\tau, s) = \frac{\partial f}{\partial y}(\tau, B^H_{\tau, s})(s - \tau)^\alpha
    \]
\end{lem}
