...
Thanks to the Bichteler-Dellacherie Theorem \cite[Theorem 47]{Protter2005}, a semi-martingale is characterized as a process $\{Y_t\}_{t\in I}$ which can be decomposable as a sum $Y_t = F_t + Z_t$ of an FV process $\{F_t\}_{t\in I}$ and a local martingale process $\{Z_t\}_{t\in I}$. An FV process is a stochastic process whose sample paths are almost surely c\`adl\`ag paths of finite variation. A function defined on a real interval is called c\`ad\`ag when it is left continuous with right limits (from the French, continue )
