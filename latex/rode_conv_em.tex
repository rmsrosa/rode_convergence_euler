...
\]
For a semi-martingale noise, the integral above can be a combination of a Riemann-Stieltjes integral, a stochastic integral, and an integral with respect to a jump measure, but we leave it like that just for the sake of explaining the main idea. With that, we bound the global error term using the Fubini Theorem, taking into consideration that, despite the inner integral being from $\xi$ to the ``future'' $\tau^N(\xi) + \Delta t_N$, the integrand is still non-anticipative, 
\begin{multline*}
...
\]
Since $[Y, Y]_t^c$ is continuous and increasing, the corresponding integral is a Riemann-Stieltjes integral.
...
\begin{theorem}
    ...
    \end{equation}
    for a constant $c_0 \geq 0$; and that $f=f(t, x, y)$ is uniformly globally Lipschitz continuous in $x$ and is continuously differentiable in $(t, y)$, with bounded differentials $\partial_t f$ and $D_y f$, i.e.
    \begin{equation}
        \label{ftfylineargrowthcadlagfv}
        \left\|\partial_t f(t, x, y)\right\| \leq c_1 + c_2 \|x\|, \quad \left\|D_y f(t, x, y)\right\| \leq c_4 + c_5\|x\|,
    \end{equation}
    in $(t, x, y)\in I\times \mathbb{R}^d\times \mathbb{R}^k$, for suitable constants $c_1, c_2, c_4, c_5 \geq 0$. Assume, further, that $\{Y_t\}_{t\in I}$ is a semi-martingale. Then, the Euler scheme is strongly convergent of order 1, i.e.
    \begin{equation}
        \label{ordercadlagfv}
        \max_{j=0, \ldots, N} \mathbb{E}\left[\left\| X_{t_j}(\omega) - X_{t_j}^N(\omega) \right\|\right] \leq c \Delta t_N, \qquad \forall N \in \mathbb{N},
    \end{equation}
