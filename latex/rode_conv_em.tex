...
\title[Strong convergence order of Euler for Random ODEs]{Improved error estimate for the order of strong convergence of the Euler method for random ordinary differential equations}
...
\end{equation}
where $\mathbb{E}[\cdot]$ indicates the expectation of a random variable on $\Omega$ and $\|\cdot\|$ is the norm in the appropriate space.
...
The first main idea of the proof is to not estimate the local error and, instead, work with an explicit formula for the global error, namely (see \eqref{lemglobalerrorintegralformula})
\begin{equation}
...
\begin{align*}
    \mathbb{E}\left[\left| \int_0^{t_j} (\tau^N(\xi) + \Delta t_N - \xi) \;\mathrm{d} F_\xi \right|\right] & \leq \int_0^{t_j} (\tau^N(\xi) + \Delta t_N - \xi) \mathbb{E}[|A_\xi|] \;\mathrm{d}\xi \\ 
    & \quad + \left( \int_0^{t_j} (\tau^N(\xi) + \Delta t_N - \xi)^2 \mathbb{E}[B_\xi^2]\;\mathrm{d} \xi\right)^{1/2} \\ 
...
These two cases are treated in \eqref{secmonotonicbound} and \eqref{secItonoise}. The case of bounded variation is treated in \eqref{secmonotonicbound}, while the It\^o noise case is treated in \eqref{secItonoise} (see \eqref{lemItostep} and \eqref{thmItostep}).

The core results in these sections are \eqref{lemmonotonicbound} and \eqref{thmmonotonicbound}, for the bounded variation case, and \eqref{lemItostep} and \eqref{thmItostep}, for the It\^o noise case. The conditions in such results, however, are not readily verifiable. With that in mind, \eqref{thmdiffmonotonicbound} and \eqref{thmItonoise} give more explicit conditions for each of these two cases. Essentially, $f=f(t, x, y)$ is required to have minimal regularity in the sense of differentiability and growth conditions and the noise $\{Y_t\}_{t\in I}$ is either required to have sample paths of bounded variation or to be an It\^o noise.

These two types of noises can also appear at same time, in a given equation or system of equations, as treated in \eqref{secmixed}. This can be regarded as a vector-valued noise, where the components of the noise may either be of bounded variation or of It\^o type. See \eqref{thmmixedcase} and \eqref{thmmixedcasepractical}.
...
\begin{lem}
    Under \eqref{standinghypotheses1}, suppose further that
    \begin{equation}
...
\begin{lem}
    ...
    Under \eqref{standinghypotheses1}, the Euler approximation \eqref{emscheme} for any pathwise solution of the random ordinary differential equation \eqref{rodeeq} satisfies the global error formula
    \begin{equation}
...
\begin{proof}
    Under \eqref{standinghypotheses1}, the solutions of \eqref{rodeeq} are pathwise solutions in the Lebesgue sense of \eqref{integralrodeform}. With that in mind, we first obtain an expression for a single time step, from time $t_{j-1}$ to $t_j = t_{j-1} + \Delta t_N$.
...
\begin{rmk}
    Strictly speaking, we only need condition \eqref{standinghypotheses1Car} from \eqref{standinghypotheses1} in order to deduce \eqref{Etjbasicbound}, but since we need \eqref{standinghypotheses1Lx} for the strong convergence anyways, it is simpler to state the result as in \eqref{lembasicestimate}.
\end{rmk}
...
\begin{lem}
    ...
    Under \eqref{standinghypotheses1}, the global error \eqref{globalerrorintegralformula} is estimated as
    \begin{equation}
        \label{Etjbasicbound}
        \begin{aligned}
            \|X_{t_j} - X_{t_j}^N\| & \leq \left( \|X_0 - X_0^N\| + L_X \int_0^{t_j} \|X_s - X_{\tau^N(s)}\| \;\mathrm{d}s \right. \\
...
\begin{lem}
    ...
    Under \eqref{standinghypotheses1}, it follows that
    \begin{equation}
...
\begin{prop}
    ...
    Under \eqref{standinghypotheses1}, suppose further that \eqref{EX0strongbound} and \eqref{EMtstrongbound} hold and that, for some constant $C_0 \geq 0$, 
    \begin{equation}
        \label{EX0X0N}
...
\begin{proof}
    Estimate \eqref{expectedestimateglobalerrorintegral} is obtained by taking the expectation of \eqref{Etjbasicbound} in \eqref{lembasicestimate} and properly estimating the first two terms on the right hand side. The first term is handled with the assumption \eqref{EX0X0N}. We just need to take care of the second term.
    
    Under \eqref{standinghypotheses1}, \eqref{estimatesecondterminglobalerror} applies and estimate \eqref{estimatesecondterminglobalerrorintegral} holds.
    ...
    Now we look at \eqref{lembasicestimate}. Taking the expectation of the global error formula \eqref{Etjbasicbound} gives
    \begin{multline*}
...
Here, the noise $\{Y_t\}_{t\in I}$ is \emph{not} assumed to be an It\^o noise, but, instead, that the steps can be controlled by monotonic nondecreasing processes with finite expected growth. This fits well the typical case of point processes, such as renewal-reward processes, Hawkes process, and the like.

More precisely, we have the following result:
\begin{lem}
    \label{lemmonotonicbound}
    Besides \eqref{standinghypotheses1}, suppose that, for all $0 \leq s \leq T$,
    \begin{equation}
        \label{stepbound}
...
\begin{thm}
    ...
    Under \eqref{standinghypotheses1}, suppose also that
...
\begin{proof}
    Under \eqref{standinghypotheses1}, the \eqref{lembasicestimate} applies and the global error estimate \eqref{Etjbasicbound} holds.
    
    Thanks to \eqref{EX0strongbound}, \eqref{EMtstrongbound}, and \eqref{EX0X0N}, the \eqref{propbasicestimate} applies and the global error is bounded according to \eqref{expectedestimateglobalerrorintegral}.
    
    With assumptions \eqref{stepbound} and \eqref{expectstepmonotonic}, \eqref{lemmonotonicbound} applies and the last term in \eqref{expectedestimateglobalerrorintegral} is bounded according to \eqref{expectintfboundbyG}. Using \eqref{expectintfboundbyG} in \eqref{expectedestimateglobalerrorintegral} yields
    \begin{multline*}
...
The conditions of \eqref{thmmonotonicbound}, especially \eqref{stepbound}-\eqref{expectstepmonotonic}, are not readily verifiable, but the following result gives more explicit conditions.
...
\begin{proof}
    ...
    Now, in order to apply \eqref{thmmonotonicbound}, it remains to verify \eqref{stepbound}-\eqref{expectstepmonotonic}. By the hypothesis on the differentiability of $f(t, x, y)$ with respect to $(t, y)$, we have
    \[
    ...
    \]
    Thus, \eqref{thmmonotonicbound} applies and we deduce the strong order 1 convergence of the Euler method.
\end{proof}
...
\begin{rmk}
    ...
    \]
    With that, the required conditions on $\{\bar F_t\}_{t\in I}$ are met and we are allowed to apply \eqref{thmmonotonicbound} and deduce the strong order 1 convergence of the Euler method.
\end{rmk}
...
\begin{rmk}
    ...
    \]
    Since the partial derivatives of $a=a(t, y)$ and $b=b(t, y)$ are uniformly bounded and $h$ is Lipschitz, it follows that the partial derivatives $\partial_t f$ and $\partial_y f$ have at most linear growth. Thus, \eqref{ftfylineargrowth} is satisfied and \eqref{thmdiffmonotonicbound} applies. Similarly for the vector-values cases. But this special form \eqref{lineareqform} is by no means necessary, and the result applies to more general terms $f=f(t, x, y)$, as stated in the theorem.
\end{rmk}
...
\begin{lem}
    \label{lemItostep}
    Besides \eqref{standinghypotheses1}, suppose that $F_t^N = f(t, X_{\tau^N(t)}^N, Y_t)$ is an It\^o noise, satisfying
    \begin{equation}
        \label{Itostep}
...
Combining the estimate in \eqref{lemItostep} with the previous estimate for the global error we obtain the following main result.
\begin{thm}
    \label{thmItostep}
    Under \eqref{standinghypotheses1}, suppose also that
...
\begin{proof}
    ...
    With assumptions \eqref{Itostep} and \eqref{expectItostepterms}, \eqref{lemItostep} applies and the last term in \eqref{expectedestimateglobalerrorintegral} is bounded according to \eqref{expectintfboundbyIto}. Using \eqref{expectintfboundbyIto} in \eqref{expectedestimateglobalerrorintegral} yields
    \begin{multline*}
...
\begin{proof}
    Let us start by showing that \eqref{standinghypotheses1} is valid. Since $f=f(t, x, y)$ is (twice) continuously differentiable with, in particular, bounded derivative in $x$, then it is uniformly globally Lipschitz in $x$. Since $a=a(t, y)$ and $b=b(t, y)$ are continuous, the the noise has continuous sample paths. Thus, the remaining condition in \eqref{standinghypotheses1} to be verified is \eqref{standinghypotheses1Carboundonf}.
    ...
    Therefore, all the conditions of \eqref{thmItostep} are met and we deduce the strong order 1 convergence of the Euler method.
\end{proof}

\begin{rmk}
    When the diffusion term $b = b(t, y) = b(t)$, in \eqref{YtItonoise}, is actually independent of $y$, then the noise is an additive noise and in this case the Euler scheme is well known to be of strong order 1 \cite{HighamKloeden2021}. In the more general $b=b(t, y)$ case, however, the Euler scheme has always been regarded to be of order 1/2 \cite{GruneKloeden2001} (see also \cite{WangCaoHanKloeden2021} for mean square convergence). Here, though, we deduce, under the conditions of \eqref{thmItonoise}, that even if $b=b(t, y)$ depends on $y$, the strong convergence of the Euler scheme is actually of order 1.    
\end{rmk}

\section{The mixed case with It\^o and bounded variation noises}
\label{secmixed}

Of course, it is possible to mix the two cases and have the following result combining \eqref{thmmonotonicbound} and \eqref{thmItostep}.
...
We ommit the proof since it is just a combination of \eqref{lemmonotonicbound} and \eqref{lemItostep}. As a consequence, we also have the following more explicit result, which is a combination of \eqref{thmdiffmonotonicbound} and \eqref{thmItonoise}.
...
\begin{enumerate}
    ...
    \item A number $N_{\mathrm{tgt}}$ of mesh points for a finer discretization to compute a target solution path.
    \item The target solution path is either a pathwise solution with an exact distribution, when available, or a higher-order approximation, thanks to the choice of $N_{\mathrm{tgt}}$. The target resolution is typically $N_{\mathrm{tgt}} = \max_i\{N_i^2\}$, e.g. $N=2^{20} = 1,048,576$, unless an exact pathwise solution is available, in which case a coarser mesh of the order of can be used.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Then, for each time step $N_i = 2^{n_i}$ in the selected range, we compute the Euler approximation using the computed noise values at the corresponding coarser mesh.
...
Julia has a performant and feature-rich \texttt{DifferentialEquations.jl} ecosystem of packages for solving differential equations \cite{RackauckasNie2017}, including random and stochastic differential equations, as well as delay equations, differential-algebraic equations, jump diffusions, partial differential equations, neural differential equations. It also has packages to seemlessly compose such equations in optimization problems, Bayesian parameter estimation, global sensitivity analysis, uncertainty quantification, and so on.

Although all the source code for the \texttt{DifferentialEquations.jl} ecosystem is available on the Github platform \cite{github}, it involves a quite large ecosystem of packages, with an intricate interplay between them. Hence, for the numerical results presented below, we chose not to use this ecosystem and, instead, implement our own code, with a minimum set of methods necessary for the convergence estimates. This is done mostly for the sake of transparency, in such a way that checking the accuracy of the implementation, for publication purposes, would be easier. All the source code for the numerical simulations presented below are in a Github repository \cite{RODEConvEM2023}.
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_wiener_linearhomogenous.png}
...
Now we consider a system of linear equations with a series of different types of noises. For most of these noises, the current knowledge expects a lower order of strong convergence than the strong order 1 we prove here. The aim of this section is to illustrate this improvement at once, for all such noises.
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_allnoises.png}
...
\end{equation}
computing a distributionally exact solution of this form is a delicate process. Thus we check the convergence numerically by solving the equation with the Euler method itself, but on a much finer mesh. Nevertheless, the proof is done rigorously below, with the framework developed in the first sections.
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_dep_on_H_fBm.png}
...
\]
where $\gamma > 0,$ $0 < \varepsilon < 1,$ and $\{Z_t\}_{t\geq 0}$ is a geometric Brownian motion (gBm) process, hence of the form \eqref{YtItonoise}-\eqref{Itonoiseterms}. A Wiener process is a more natural choice, but we choose a gBm process instead since it is a multiplicative noise, thus the convergence would not be expected to be of strong order 1 based on previous works.
...
The function $f=f(t, x, y)$ is continuously differentiable infinitely many times and with bounded derivatives within the positively invariant interval. Hence, within the region of interest, all the conditions of \eqref{thmmixedcasepractical} hold and the Euler method is of strong order 1.
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_popdyn_gBmPoisson.png}
    \caption{Order of convergence of the strong error of the Euler method for equation \eqref{rodepopulationdynamics}.}
    \label{figpopdyn}
\end{figure}

\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/sample_popdyn_gBmPoisson.png}
...
\subsection{Mechanical structural under random Earthquake-like seismic disturbances}
...
\begin{table}
    ...
    \caption{Mesh points (N), time steps (dt), and strong error (error) of the Euler method for Kanai-Tajimi Earthquake model, with initial condition $X_0 = \mathbf{0}$ and Transport process, on the time interval (0.0, 2.0), based on $m = 500$ sample paths for each fixed time step, with the target solution calculated with $262144$ points.}
    \label{tableearthquake}
\end{table}

\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/convergence_earthquake.png}
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_toggleswitch.png}
    \caption{Order of convergence of the strong error of the Euler method for the mechanical structure model \eqref{mechanicalstructuremodel}.}
...
Here we simulate a Fisher-KPP equation with random boundary conditions, as inspired by the works of \cite{SalakoShen2020}  and \cite{FreidlinWentzell1992}. The first work addresses the Fisher-KPP equation with a random reaction coefficient, while the second work considers more general reaction-diffusion equations but driven by random boundary conditions. The deterministic Fisher-KPP equations has its origins in \cite{Fisher1937} and \cite{KPP1937}

We consider the Fisher-KPP equation driven by Neumann boundary conditions, with a random influx on the left end point and no flux on the right end point. The intent here is to illustrate the strong order 1 convergence rate on a discretization of a nonlinear partial differential equation.
...
For the time-mesh parameters, we set $N_{\textrm{tgt}} = 2^{15} \times 3^3 \times 5 = 4,423,680$ and $N_i = 2^{10}, 2^7 \times 3^2, 2^8 \times 5, 2^9 \times 3, 2^7 \times 3 \times 5, 2^{11}$. The reason for not using powers of two is because, due to the space discretization, we cannot use too coarse a time mesh, for stability reasons, limiting the range of time-mesh parameters. Instead of going too high and have too slow computations, we populate the range of $N_i$'s with values which are not power of two.
...
\begin{figure}[htb]
    \includegraphics[scale=0.5]{img/order_fisherkpp.png}
    \caption{Order of convergence of the strong error of the Euler method for the mechanical structure model \eqref{mechanicalstructuremodel}.}
    \label{figfisherkpp}
\end{figure}
...
\begin{thebibliography}{25}
    ...
    \bibitem{github} github, \emph{GitHub,} available at \texttt{https://github.com/}, 2020.
