...
In this section, we illustrate the strong order 1 convergence with a few examples that fall into one of the cases considered above, starting with a system of linear equations with different types of noises. We also illustrate the $H + 1/2$ order of convergence in the case of a fractional Brownian motion (fBm) noise with Hurst parameter $0 < H < 1/2$. Then we implement a nonlinear toogle-switch model of gene expression. As mentioned in the Introduction, further examples can be found in the github repository \cite{RODEConvEM2023}.
