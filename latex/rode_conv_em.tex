...
Here, we use the \textbf{fractional It\^o formula} \cite[Theorem 4.2.6]{BHOB2008}, which says that, for a fractional Brownian motion $\{B^H_t\}_t$ with $0 < H < 1$ and for $f=f(t, y)$ continuous along with its derivatives up to order one in $t$ and two in $y$,
\begin{multline*}
    f(t, B^H_t) = f(s, B^H_s) + \int_s^t \frac{\partial f}{\partial }(s, B^H_s)\;\mathrm{d}s \\
        + \int_s^t \frac{\partial f}{\partial y}(s, B^H_s)\;\mathrm{d}B^H_s + H \int_0^t \frac{\partial^2 f}{\partial y^2}(s, B^H_s)s^{2H - 1}\;\mathrm{d}s,
\end{multline*}
where the integral with respect to $B^H_s$ is a Wick It\^o Skorohod (WIS) integral (see \cite[Chapter 4]{BHOB2008}), 
...
Another fundamental result is the \textbf{WIS isometry} \cite[Theorem 4.5.6]{BHOB2008}, which says that
