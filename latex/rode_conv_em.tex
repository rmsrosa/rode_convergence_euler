...
\]
As a function of $s$, $B^H_{s, t}$ is non-anticipative with respect to the Wiener process and is an It\^o process within the interval $0 < s < t$, with
\begin{multline*}
    \mathrm{d}_s B^H_{s, t} = (H-1/2)C_H s^{H-3/2}\left(\int_0^s \tau^{1/2 - H}(t - \tau)^{H-1/2}\;\mathrm{d}B_\tau\right)\;\mathrm{d}s \\
    + C_H (t - s)^{H-1/2}\;\mathrm{d}B_s.
\end{multline*}

Hence, using the It\^o formula,
\begin{multline*}
    f(s, B^H_{s, t}) \\
    = 
\end{multline*}

In order to avoid the singularity, we first look within the interval $\varepsilon < s < t - \varepsilon$. Within this interval,

\[
    B_{t-\varepsilon,t} - B_{\varepsilon,t} = \int_\varepsilon^{t-\varepsilon} \left(\frac{\partial f(t, B_{\tau, t})}{\partial s} + \frac{\partial f(t, B_{\tau, t})}{\partial y}\frac{\partial B_{\tau, t}}{\partial \tau}\right)\;\mathrm{d}\tau
\]

Therefore,
\[
    B^H_t = B^H_{t, t} = B^H_{0, t} + \int_0^t \mathrm{d}B^H_{s, t} = \int_0^t 
\]
