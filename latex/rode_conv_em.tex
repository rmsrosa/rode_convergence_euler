...
\begin{lem}
    ...
    \begin{equation}
        \label{defBHst}
        Y_{s, t} = s^\alpha Z_{s, t} - \alpha \int_0^s \eta^{\alpha-1}Z_{\eta, \eta}\;\mathrm{d}\eta.
    \end{equation}
...
\[
    m_H(t, s) = C_H \left( \left(\frac{t}{s}\right)^\alpha (t - s)^\alpha - \alpha s^{-\alpha}\int_s^t\eta^{\alpha-1}(\eta - s)^\alpha\;\mathrm{d}\eta\right),
\]
...
\[
    \mathbb{E}\left[(Z_{s, t})^2\right] = C_H^2 \int_0^s \tau^{-2\alpha}(t - \eta)^{2\alpha}\;\mathrm{d}\eta \\
...
\begin{lem}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            & f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \\
            & \quad = \int_{\tau^N(s)}^{s} G(\eta, s) \;\mathrm{d}\eta + \int_0^{\tau^N(s)} (G(\eta, s) - G(\eta, \tau^N(s))) \;\mathrm{d}\eta \\
            & \quad + \int_{\tau^N(s)}^{s} H(\eta, s) \;\mathrm{d}W_\eta + \int_0^{\tau^N(s)} (H(\eta, s) - H(\eta, \tau^N(s))) \;\mathrm{d}W_\eta,
        \end{aligned}
    \end{equation}
...
\begin{proof}
    ...
    \begin{align*}
        f(s, Y_s) & = f(0, Y_0) + \int_{0}^s \frac{\partial f}{\partial \eta}(\eta, X_{\tau^N(s)}^N, B_{\eta, s})\;\mathrm{d}\eta \\ 
        & \quad + \alpha \int_{0}^s \frac{\partial f}{\partial y}(\eta, X_{\tau^N(s)}^N, B_{\eta, s})\eta^{\alpha - 1} (Z_{\eta, s} - Z_{\eta, \eta})\;\mathrm{d}\eta \\
        & \quad + \frac{C_H^2}{2}\int_{0}^s \frac{\partial^2 f}{\partial y^2}(\eta, X_{\tau^N(s)}^N, B_{\eta, s}) (s - \eta)^{2\alpha} \;\mathrm{d}\eta \\
        & \quad + C_H \int_{0}^s \frac{\partial f}{\partial y}(\eta, X_{\tau^N(s)}^N, B_{\eta, s})(s - \eta)^\alpha\;\mathrm{d} W_\eta.
    \end{align*}
    In terms of $G$ and $H$, this becomes
    \[
        f(s, Y_s) = f(0, Y_0) + \int_0^s G(\eta, s)\;\mathrm{d}\eta + \int_0^s H(\eta, s)\;\mathrm{d}W_{\eta}.
    \]
...
\begin{proof}
    ...
    \begin{align*}
        & \int_0^t \left(f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})\right)\;\mathrm{d}s \\
        & \quad = \int_0^t\int_{\tau^N(s)}^{s} G(\eta, s) \;\mathrm{d}\eta\;\mathrm{d}s + \int_0^t\int_0^{\tau^N(s)} (G(\eta, s) - G(\eta, \tau^N(s))) \;\mathrm{d}\eta\;\mathrm{d}s \\
        & \quad + \int_0^t\int_{\tau^N(s)}^{s} H(\eta, s) \;\mathrm{d}W_\eta\;\mathrm{d}s + \int_0^t\int_0^{\tau^N(s)} (H(\eta, s) - H(\eta, \tau^N(s))) \;\mathrm{d}W_\eta\;\mathrm{d}s.
    \end{align*}
    Using Fubini's Theorem,
    \begin{align*}
        & \int_0^t \left(f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})\right)\;\mathrm{d}s \\
        & \quad = \int_0^t\int_\eta^{\tau^N(\eta) + \Delta t_N} G(\eta, s) \;\mathrm{d}s\;\mathrm{d}\eta + \int_0^t\int_{\tau^N(\eta) + \Delta t_N}^t (G(\eta, s) - G(\eta, \tau^N(s))) \;\mathrm{d}s\;\mathrm{d}\eta \\
        & \quad + \int_0^t\int_\eta^{\tau^N(\eta) + \Delta t_N} H(\eta, s) \;\mathrm{d}s\;\mathrm{d}W_\eta + \int_0^t\int_{\tau^N(\eta) + \Delta t_N}^t (H(\eta, s) - H(\eta, \tau^N(s))) \;\mathrm{d}s\;\mathrm{d}W_\eta.
    \end{align*}
