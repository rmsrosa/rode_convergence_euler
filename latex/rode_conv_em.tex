...
For the theory of semi-martingales, we refer the reader to \cite{Protter2005} and \cite{Metivier1982}. The main idea is that semi-martingales are stochastic processes $\{Y_t\}_{t\geq 0}$ which are c\`adl\`ag, adapted, and ``good integrators,'' in the sense that the sums
\[
...
Another important process associated with a semi-martingale is the \emph{quadratic variation} $\{[Y, Y]_t\}_{t\in I}$ defined as the limit of the summations of quadratic increments as the mesh is refined,
\[
    [Y, Y]_t = \lim_{\substack{t_0 < \ldots < t_n \in [0, t], \;n\in \mathbb{N} \\ \max_{i=1, \ldots, n}|t_i - t_{i-1}| \rightarrow 0}} \sum_{i=1}^n (Y_{t_i} - Y_{t_{i-1}})^2,
\]
or, equivalently, by the formula (which is a particular case of an ``integration by parts" formula for the \emph{quadratic covariation process} $\{[Y, \tilde Y]_t\}_{t\in I}$ between two-semimartingales $\{Y_t\}_t$ and $\{\tilde Y_t\}_t$)
\[
