...
For the time-mesh parameters, we set $N_{\textrm{tgt}} = 2^{18}$ and $N_i = 2^5, 2^7, 2^9$. The spatial discretization is done with finite differences with the number of mesh points depending on the time mesh, for stability reasons. Thus, for each $N_i$, we take $2^3+1=9, 2^4+1=17,$ and $2^5+1=33$ spatial points, respectively, while for $N_{\textrm{tgt}}$, we use $2^9+1 = 513$ spatial points.
