...
\]
The quadratic variation is a c\`adl\`ag, increasing, and adapted process \cite[Theorem II.22]{Protter2005}. Notice that $[Y, Y]_0 = Y_0^2.$ The continuous part is given by
\[
    [Y, Y]_t^c = [Y, Y]_t - \sum_{0\leq s \leq t} \left(\Delta Y_s\right)^2.
\]
Since $[Y, Y]_t^c$ is continuous and increasing, the corresponding integral is a Lebesgue-Stieltjes integral. For the continuous part, $[Y, Y]_t^0 = [Y, Y]_0 - \left(\Delta Y_0\right)^2 = Y_0^2 - Y_0^2 = 0.$
...
\begin{theorem}
    ...
    \begin{align}
        ...
        & \sup_{t\in I} \mathbb{E}\left[\|Z_t\|\right] < \infty.
    \end{align}   
...
\begin{proof}
    ...
    \begin{equation}
        \begin{aligned}
            \mathbb{E}\bigg[\bigg\| & \int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s \bigg\|\bigg] \\
            & \leq \;\Delta t_N \Bigg( c_1 t_j + \frac{c_2}{c_L} \left(\mathbb{E}\left[\|X_0\|\right] + c_0t_j + \mathbb{E}[C_M] t_j \right) e^{c_L t_j} \\
            & \qquad \qquad + c_3\sup_{0 \leq \xi \leq t_j}\mathbb{E}\left[\|Y_{\xi}\|\right] + c_4  \mathbb{E}\left[ V(\{F_t\}_{t\in I}; [0, t_j]) \right] + c_4\mathbb{E}\left[ [Z, Z]_{t_j}\right] \\
            & \qquad \qquad + \frac{c_7}{2} \mathbb{E}\left[\left\|\sum_{0 < \xi \leq t_j}\left( \Delta Y_\xi \right)^2\right\|\right] + \frac{c_7}{2} \mathbb{E}\left[[Y, Y]_{t_j}^c\right] \Bigg).
        \end{aligned}
    \end{equation}
\end{proof}
