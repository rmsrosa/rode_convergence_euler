...
First we simulate a linear equation with several types of noises, emcompassing noises with sample paths with bounded variation and It\^o process noises. Next we simulate a linear equation with fBm noise. Then we implement a nonlinear toogle-switch model of gene expression driven by a combination of a compound Poisson process and geometric Brownian motion process, illustrating the two main types of noises considered here, namely a point-process with bounded variation and a multiplicative It\^o process noise.
