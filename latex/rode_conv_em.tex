...
\begin{multline}
    \label{Etjbasicboundintro}
        \|X_{t_j} - X_{t_j}^N\| \leq \left( \|X_0 - X_0^N\| + \ell \int_0^{t_j} \|X_s - X_{\tau^N(s)}\| \;\mathrm{d}s \right. \\
        \left. \left\|\int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right\|\right) e^{\ell t_j}.
\end{multline}
...
\begin{lemma}
    ...
    \end{equation}
    and that $\{L_t\}_{t\in I}$ is essentially bounded on $\Omega,$ uniformly on $I$, i.e. there exists a constant $\ell > 0$ such that
    \begin{equation}
        \label{LtLXbound}
        L_t \leq \ell, \quad  t\in I,
    \end{equation}
    almost surely. Then
    \begin{equation}
        \label{EXtstrongbound}
        \mathbb{E}[\|X_t\|] \leq \left(\mathbb{E}[\|X_0\|] + \int_0^t \mathbb{E}[M_s]\;\mathrm{d}s\right) e^{\ell t}, \quad t\in I.
    \end{equation}
\end{lemma}
...
\begin{remark}
    ...
    \begin{equation}
        ...
        \mathbb{E}[\|X_t\|^\beta] \leq \left(\mathbb{E}[\|X_0\|^\beta] + \int_0^t \mathbb{E}[M_s^\beta]\;\mathrm{d}s\right) e^{((\beta - 1) + \beta \ell) t}, \quad t\in I.
    \end{equation}
\end{remark}
...
\section{Bound on the pathwise approximation}
...
\end{equation}
The right hand side, however, depends on the time mesh, so we need to estimate it properly. A straighforward estimate is obtained from assuming that $\{L_t\}_{t\in I}$ and $\{M_t\}_{t\in I}$ are essentially bounded.
\begin{lemma}
    \label{lemXNtboundLXMt}
    Under the \cref{standinghypotheses1}, assume moreover that $\{L_t\}_{t\in I}$ and $\{M_t\}_{t\in I}$ are essentially bounded, uniformly on $I$. Then, the Euler approximation \eqref{emscheme} for a pathwise solution of the random ordinary differential equation \eqref{rodeeq} satisfies almost surely
    \begin{equation}
        \label{XNtboundLXMt}
        \|X_{t_j}^N\| \leq \left(\|X_0^N\| + \left(\operatorname{ess\;sup}_{t \in I} M_t \right) t_j \right)e^{\left(\operatorname{ess\;sup}_{t \in I} L_t \right) t_j}, \quad \forall j = 0, \ldots, N.
    \end{equation}
    \qed
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        \int_0^{t_j}L_s\left\|X_s - X_{\tau^N(s)}\right\| \;\mathrm{d}s & \leq \int_0^{t_j}L_s\int_{\tau^N(s)}^s (M_\xi + \ell\|X_\xi\|) \;\mathrm{d}\xi \;\mathrm{d}s \\
...
\begin{proposition}
    ...
    \begin{multline}
        \label{expectedestimateglobalerrorintegral}
            \mathbb{E} \left[\|X_{t_j} - X_{t_j}^N\|\right] \leq \left( C_0 \Delta t_N + \Delta t_N \ell \left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M_\xi]\;\mathrm{d}\xi\right)e^{\ell t_j}\right. \\
            \left. \mathbb{E}\left[\left\|\int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right\|\right]\right) e^{\ell t_j}.
    \end{multline}
\end{proposition}
...
\begin{proof}
    ...
    \[
        \int_0^{t_j} L_s \mathbb{E}[\|X_s - X_{\tau^N(s)}\|] \;\mathrm{d}s \leq \Delta t_N \ell \int_0^{t_j} (\mathbb{E}[M_s] + \ell\mathbb{E}[\|X_s\|]) \;\mathrm{d}s.
    \]
    ...
    \begin{align*}
        ...
        & \leq \Delta t_N \ell \int_0^{t_j} \left(\mathbb{E}[M_s] + \ell\left(\mathbb{E}[\|X_0\|] + \int_0^s \mathbb{E}[M_\xi]\;\mathrm{d}\xi\right)e^{\ell s} \right)\;\mathrm{d}s \\
        & \leq \Delta t_N \ell \left(\int_0^{t_j} \mathbb{E}[M_s] \;\mathrm{d}s + \ell \int_0^{t_j}\left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M_\xi]\;\mathrm{d}\xi\right)e^{\ell s} \;\mathrm{d}s\right) \\
        & = \Delta t_N \ell \left(\int_0^{t_j} \mathbb{E}[M_s] \;\mathrm{d}s + \left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M_\xi]\;\mathrm{d}\xi\right)\left(e^{\ell t_j} - 1\right) \right).
    \end{align*}
    Thus,
    \begin{equation}
        \label{expectedestimatesecondterminglobalerrorintegral}
        \int_0^{t_j} L_s \mathbb{E}[\|X_s - X_{\tau^N(s)}\|] \;\mathrm{d}s \leq \Delta t_N \ell \left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M_\xi]\;\mathrm{d}\xi\right)e^{\ell t_j}.
    \end{equation}
...
\begin{theorem}
    ...
    \end{equation}
    for a random variable $C_0$ which is finite almost surely; and that $f=f(t, x, y)$ is uniformly globally Lipschitz continuous in $x$ and is continuously differentiable in $(t, y)$, with differentials $\partial_t f$ and $D_y f$ with at most linear growth in $x$ and $y$, i.e.
    \begin{equation}
...
\begin{proof}
    ...
    \begin{align*}
        & \left\|\int_0^{t_j} \sum_{\tau^N(s) < \xi \leq s} \left(f(\xi, X_{\tau^N(s)}^N, Y_\xi) - f(\xi, X_{\tau^N(s)}^N, Y_{\xi^{-}}) - D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\Delta Y_\xi\right)\;\mathrm{d}s\right\| \\
        & \qquad \leq \sum_{0 < \xi \leq t_j} \int_{\xi}^{\tau(\xi)+\Delta t} 2\left(c_4 + c_5\|X_{\tau^N(s)}^N\| + c_6\max\{\|Y_{\xi^{-}}\|, \|Y_{\xi}\|\}\right)\|\Delta Y_\xi\|\;\mathrm{d}s \\
        & \qquad \leq 2 \sum_{0 < \xi \leq t_j} \left(\int_{\xi}^{\tau(\xi)+\Delta t} \left(c_4 + c_5\|X_{\tau^N(s)}^N\| + c_6\max\{\|Y_{\xi^{-}}\|, \|Y_{\xi}\|\}\right)\;\mathrm{d}s \right)\|\Delta Y_\xi\| \\
        & \qquad \leq 2\left(c_4 + c_5\max_{i=0, \ldots, j}\|X_i^N\| + c_6\sup_{0\leq s \leq t_j}\|Y_s\|\right) \Delta t \sum_{0 < \xi \leq t_j} \|\Delta Y_\xi\| \\
        & \qquad \leq 2\left(c_4 + c_5\max_{i=0, \ldots, j}\|X_i^N\| + c_6\sup_{0\leq s \leq t_j}\|Y_s\|\right) V(\{Y_t\}_{t\in I}; [0, t_j]) \Delta t.
    \end{align*}
    ...
    \begin{multline}
        ...
        + 2\left(c_4 + c_5\max_{i=0, \ldots, N}\|X_i^N\| + c_6\sup_{0\leq s \leq T}\|Y_s\|\right) V(\{Y_t\}_{t\in I}; [0, t_j]).
    \end{multline}
    ...
    \comment{Wait, the constant $C=C(\omega)$ cannot depend on the solution. Need to use \eqref{XNtboundLXMt} on the estimates above.}
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        & \quad \leq \frac{1}{2}\sum_{0 < \xi \leq t_j}\int_\xi^{\tau^N(\xi) + \Delta t} \left(c_7 + c_8 \|X_{\tau^N(\xi)}^N\| + c_9 \max\{\|Y_\xi\|, \|Y_{\xi^-}\|\} \right)\left( \Delta Y_\xi \right)^2\;\mathrm{d}s \\
        & \quad \leq \frac{\Delta t}{2} \left( c_7 + c_8 \max_{i=0, \ldots, j}\|X_{t_i}^N\| + c_9 \sup_{0\leq \xi \leq t_j}\|Y_\xi\|\right) \sum_{0 < \xi \leq t_j}\left( \Delta Y_\xi \right)^2.
    \end{align*}
    ...
    \begin{align*}
        ...
        & \qquad \leq \frac{1}{2} \int_0^{t_j} \int_{\xi}^{\tau^N(\xi) + \Delta t} \left(c_7 + c_8 \|X_{\tau^N(\xi)}^N\| + c_0\|Y_{\xi^-}\|\right)\;\mathrm{d}s\;\mathrm{d}[Y, Y]_\xi^c \\
        & \qquad \leq \frac{\Delta t}{2}\int_0^{t_j} \left(c_7 + c_8 \|X_{\tau^N(\xi)}^N\| + c_9\|Y_{\xi^-}\|\right)\;\mathrm{d}[Y, Y]_\xi^c \\ 
        & \qquad \leq \frac{\Delta t}{2} \left(c_7 + c_8 \max_{i=0, \ldots, j}\|X_{t_i}^N\| + c_9\sup_{0\leq \xi \leq t_j}\|Y_\xi\|\right) [Y, Y]_{t_j}^c.
    \end{align*}
    ...
    \begin{equation}
        \begin{aligned}
            ...
            & \quad \leq \Delta t\int_0^{t_j} \left(c_1 + c_2 \|X_{\tau^N(\xi)}^N\| + c_3\|Y_{\xi^-}\|\right)\;\mathrm{d}\xi \\
            & \qquad + \Delta t\mathbb{E}\left[\int_0^{t_j} \left(c_4 + c_5\|X_{\tau^N(\xi)}^N\| + c_6 \|Y_{\xi^-}\| \right)^2\;\mathrm{d}[Z, Z]_\xi\right] \\
            & \qquad + \frac{\Delta t}{2} \left( c_7 + c_8 \max_{i=0, \ldots, j}\|X_{t_i}^N\| + c_9 \sup_{0\leq \xi \leq t_j}\|Y_\xi\|\right) \sum_{0 < \xi \leq t_j}\left( \Delta Y_\xi \right)^2 \\
            & \qquad + \frac{\Delta t}{2} \left(c_7 + c_8 \max_{i=0, \ldots, j}\|X_{t_i}^N\| + c_9\sup_{0\leq \xi \leq t_j}\|Y_\xi\|\right) [Y, Y]_{t_j}^c.
        \end{aligned}
    \end{equation}
\end{proof}
