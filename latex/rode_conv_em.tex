...
Here we derive an estimate that is the basis for the specific estimates for each type of noise. For that, we use a discrete version of the Grownwall Lemma. Here we state a particular case of the Discrete Grownwall Lemma that can be found in \cite{GiraultRaviart1981} (see also \cite{Clark1987}).
...
\begin{proof}
    ...
    The result is trivially true for $j=0$. Suppose, by induction, that the result is true up to $j-1$. Then, using \eqref{integralgronwall} for $j$ and the induction hypothesis, we find
    \[
...
\begin{proof}
    ...
    \end{align*}
    Using the discrete version of the Gronwall Lemma given in \cref{lemdiscretegronwall}, we prove \eqref{Etjbasicbound}.
\end{proof}
