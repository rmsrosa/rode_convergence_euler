...
\]
for a suitable standard Wiener process $\{W_s\}_s$, where
\[
    m_H(t, s) = C_H \left( \left(\frac{t}{s}\right)^\alpha (t - s)^\alpha - \alpha s^{-\alpha}\int_s^t\tau^{\alpha-1}(\tau - s)^\alpha\;\mathrm{d}\tau\right)
\]
with $\alpha = H-1/2$ (for notational simplicity) and $C_H$ is given by (it is the constant $C_H^{(6)}$ in \cite[Section 1.8]{Mishura2008}, with $\alpha = H - 1/2$)
\[
    C_H = \left( \frac{2H\Gamma(1-\alpha)}{\Gamma(1 - 2\alpha)\Gamma(1+\alpha)} \right)^{1/2}.
\]

Thus, as in \cite[Section 2.7.4]{Mishura2008}, we define, for $0 \leq s \leq t$,
\[
    Y^H_{s, t} = C_H \int_0^s \tau^{-\alpha}(t - \tau)^\alpha \;\mathrm{d}W_\tau,
\]
and write $B^H_t = B^H_{t, s}$, where $B^H_{t, s}$ is given by
\[
    B^H_{s, t} = s^\alpha Y^H_{s, t} - \alpha \int_0^s \tau^{\alpha-1}Y^H_{\tau, \tau}\;\mathrm{d}\tau.
\]

As functions of $s$, both $Y^H_{s, t}$ and $B^H_{s, t}$ are non-anticipative with respect to the Wiener process and are It\^o processes, with
\[
    \mathrm{d}_s Y^H_{s, t} = C_H s^{-\alpha}(t - s)^\alpha \;\mathrm{d}W_s,
\]
and
\[
    \mathrm{d}_s B^H_{s, t} = \alpha s^{\alpha - 1} (Y^H_{s, t} - Y^H_{s, s})\;\mathrm{d}s + C_H (t - s)^\alpha \;\mathrm{d}W_s.
\]
Observe that, from the It\^o isometry,
\[
    \mathbb{E}\left[(Y^H_{s, t})^2\right] = C_H^2 \int_0^s \tau^{-2\alpha}(t - \tau)^{2\alpha}\;\mathrm{d}\tau \\
    \leq C_H^2 s^{-2\alpha}t^{2\alpha}s = C_H^2 s^{1-2\alpha}t^{2\alpha}.
\]
Hence, $\mathbb{E}[|Y^H_{s, t}|] = \mathcal{O}(s^{1/2-\alpha})$, as $s\rightarrow 0$, and the singularities at both endpoints are all integrable.

Then, using the It\^o formula,
\begin{align*}
    f(s, B^H_{s, t}) & = f(0, B^H_{0, t}) + \int_{0}^s \frac{\partial f}{\partial \tau}(\tau, B_{\tau, t})\;\mathrm{d}\tau + \alpha \int_{0}^s \frac{\partial f}{\partial y}(\tau, B_{\tau, t})\tau^{\alpha - 1} (Y^H_{\tau, t} - Y^H_{\tau, \tau})\;\mathrm{d}\tau \\
    & \quad + \frac{C_H^2}{2}\int_{0}^s \frac{\partial^2 f}{\partial y^2}(\tau, B_{\tau, t}) (t - \tau)^{2\alpha} \;\mathrm{d}\tau + C_H \int_{0}^s \frac{\partial f}{\partial y}(\tau, B_{\tau, t})(t - \tau)^\alpha\;\mathrm{d} B_\tau
\end{align*}
In particular, since $B^H_{t, t} = B^H_t$ and $B^H_{0, t} = 0 = B^H_0$,
\begin{align*}
    f(t, B^H_t) & = f(0, B^H_0) + \int_{0}^t \frac{\partial f}{\partial \tau}(t, B_{\tau, t})\;\mathrm{d}\tau + \alpha \int_{0}^t \frac{\partial f}{\partial y}(\tau, B_{\tau, t})\tau^{\alpha - 1} (Y^H_{\tau, t} - Y^H_{\tau, \tau})\;\mathrm{d}\tau \\
    & \quad + \frac{C_H^2}{2}\int_{0}^t \frac{\partial^2 f}{\partial y^2}(\tau, B_{\tau, t}) (t - \tau)^{2\alpha} \;\mathrm{d}\tau + C_H \int_{0}^t \frac{\partial f}{\partial y}(\tau, B_{\tau, t})(t - \tau)^\alpha\;\mathrm{d} B_\tau
\end{align*}

Thus, using that the partial derivatives are continuous and uniformly bounded,
\begin{align*}
    \mathbb{E}\left[\left|f(t, B^H_t) - f(0, B^H_{0})\right|\right] & \leq t + \int_{0}^t \tau^{\alpha-1}\left(\mathbb{E}\left[\left|Y^H_{\tau, t}\right|\right] + \mathbb{E}\left[\left|Y^H_{\tau, \tau}\right|\right] \right)\;\mathrm{d}\tau \\
    & \quad + \frac{C_H^2}{2}\int_{0}^t (t - \tau)^{2\alpha} \;\mathrm{d}\tau + C_H \left(\int_{0}^t (t - \tau)^{2\alpha}\;\mathrm{d} \tau\right)^{1/2} \\
    & \leq t + C_H \int_0^t \tau^{\alpha-1}\left(\tau^{1/2-\alpha}t^{\alpha} + \tau^{1/2-\alpha}\tau^{\alpha}\right)\;\mathrm{d}\tau\\ 
    & \quad + \frac{C_H^2}{2}\int_{0}^t (t - \tau)^{2\alpha} \;\mathrm{d}s + C_H \left(\int_{0}^t (t - \tau)^{2\alpha}\;\mathrm{d} \tau\right)^{1/2}.
\end{align*}

But we actually need to estimate a step $f(t', B^H_{t'}) - f(t, B^H_{t})$, so we subtract
\begin{align*}
    & f(t', B^H_{t'}) - f(t, B^H_{t}) \\
    & \quad = \int_{0}^{t'} \frac{\partial f}{\partial \tau}(t', B_{\tau, t'})\;\mathrm{d}\tau - \int_{0}^t \frac{\partial f}{\partial \tau}(t, B_{\tau, t})\;\mathrm{d}\tau\\
    & \quad + \alpha \int_{0}^{t'} \frac{\partial f}{\partial y}(\tau, B_{\tau, t'})\tau^{\alpha - 1} (Y^H_{\tau, t'} - Y^H_{\tau, \tau})\;\mathrm{d}\tau - \alpha \int_{0}^t \frac{\partial f}{\partial y}(\tau, B_{\tau, t})\tau^{\alpha - 1} (Y^H_{\tau, t} - Y^H_{\tau, \tau})\;\mathrm{d}\tau\\
    & \quad + \frac{C_H^2}{2}\int_{0}^{t'} \frac{\partial^2 f}{\partial y^2}(\tau, B_{\tau, t'}) (t' - \tau)^{2\alpha} \;\mathrm{d}\tau - \frac{C_H^2}{2}\int_{0}^t \frac{\partial^2 f}{\partial y^2}(\tau, B_{\tau, t}) (t - \tau)^{2\alpha} \;\mathrm{d}\tau\\
    & \quad + C_H \int_{0}^{t'} \frac{\partial f}{\partial y}(\tau, B_{\tau, t'})(t' - \tau)^\alpha\;\mathrm{d} B_\tau - C_H \int_{0}^t \frac{\partial f}{\partial y}(\tau, B_{\tau, t})(t - \tau)^\alpha\;\mathrm{d} B_\tau
\end{align*}

This can be written in the form
\begin{align*}
    & f(t', B^H_{t'}) - f(t, B^H_{t}) \\
    & \quad = \int_{t}^{t'} G(\tau, t') \;\mathrm{d}\tau + \int_0^t (G(\tau, t') - G(\tau, t)) \;\mathrm{d}\tau \\
    & \quad = \int_{t}^{t'} H(\tau, t') \;\mathrm{d}B_\tau + \int_0^t (H(\tau, t') - H(\tau, t)) \;\mathrm{d}B_\tau,
\end{align*}
for the families of processes $\{G(\tau, t)\}_{0\leq\tau\leq t}$ and $\{H(\tau, t)\}_{0\leq\tau\leq t}$, $0\leq t \leq T$, given by
\[
    G(\tau, t) = \frac{\partial f}{\partial \tau}(t, B_{\tau, t})  + \alpha \frac{\partial f}{\partial y}(\tau, B_{\tau, t})\tau^{\alpha - 1} (Y^H_{\tau, t} - Y^H_{\tau, \tau}) + \frac{C_H^2}{2}\frac{\partial^2 f}{\partial y^2}(\tau, B_{\tau, t}) (t - \tau)^{2\alpha}
\]
and
\[
    H(\tau, t) = \frac{\partial f}{\partial y}(\tau, B_{\tau, t})(t - \tau)^\alpha
\]
