...
\begin{proof}
    ...
    \]
    % ξ = -10.0:0.05:10.0
    % K(ξ; ε = 0.2, H = 1/4) = min(max(- ξ / abs(ξ)^(3/2 - H), - 1/ε^(1/2 - H)), 1/ε^(1/2 - H))
    % plot(ξ, K, legend=nothing)
...
Observe that, from the It\^o isometry \cite{Oksendal2003},
\[
...
\begin{proof}
    ...
    \end{multline*}
    Using the Lyapunov inequality and the It\^o isometry \cite{Oksendal2003},
    \[
...
\begin{proof}
    ...
    \end{align*}
    Taking the strong norm and using the Lyapunov inequality and the It\^o isometry \cite{Oksendal2003}, we find that
    \begin{equation}
        \begin{aligned}
            & \mathbb{E}\left[\left|\int_0^t \left(f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})\right)\;\mathrm{d}s \right|\right] \\
