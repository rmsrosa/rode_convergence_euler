...
\begin{theorem}
    ...
    \end{align}
    in $(t, x, y)\in I\times \mathbb{R}^d\times \mathbb{R}^k$, for suitable constants $c_1, c_4 \geq 0$ and for suitable continuous and monotonically increasing nonnegative functions $\varphi_2, \varphi_3, \varphi_5, \varphi_6$ which vanish at zero. Assume, further, that the noise $\{Y_t\}_{t\in I}$ is an FV process. Then, the Euler scheme is pathwise convergent of order 1, i.e.
    \begin{equation}
        \label{ordercadlagfv}
...
\begin{proof}
    ...
    For the second integral on the right hand side of \eqref{sm2}, we once again switch the order of integration, then use the It\^o isometry for local martingales and then again the bound \eqref{ftfyunifboundcadlagfvpartialy}, so that
    \begin{align*}
    ...
    \end{align*}
    where we used that $[Z, Z]_0 = Z_0^2 = 0.$ We also use that, for any $0 \leq \xi \leq T,$ and for $\xi \leq s \leq \tau^N(\xi) + \Delta t_N,$ we have $\tau^N(s) = \tau^N(\xi),$ so we can write $X_{\tau^N(s)}^N = X_{\tau^N(\xi)}^N.$
