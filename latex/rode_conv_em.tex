...
\begin{theorem}
    ...
    \begin{equation}
        \label{X0conv}
        \|X_0(\omega) - X_0^N(\omega)\| \leq C_0(\omega)\Delta t,
    \end{equation}
...
\begin{proof}
    ...
    For the first term above, we have, using \eqref{ftfylineargrowthcadlagfv},
    \begin{multline*}
    ...
    \begin{multline*}
        \left\|\int_0^{t_j} \int_{\tau^N(s)}^s D_\xi f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\;\mathrm{d}\xi\;\mathrm{d}s\right\| \\
        \leq \int_0^{t_j} \int_{\xi}^{\tau^N(\xi) + \Delta t} \left(c_1 + c_2 \|X_{\tau^N(s)}^N\| + c_3\|Y_{\xi^-}\|\right)\;\mathrm{d}s\;\mathrm{d}\xi.
    \end{multline*}
    Using the bound \eqref{XNtboundLXMt} on the pathwise approximation, we obtain
    \begin{multline*}
        \left\|\int_0^{t_j} \int_{\tau^N(s)}^s D_\xi f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\;\mathrm{d}\xi\;\mathrm{d}s\right\| \\
        \leq \int_0^{t_j} \int_{\xi}^{\tau^N(\xi) + \Delta t} \left(c_1 + c_2 \left(\|X_0^N\| + C_M t_j \right)e^{C_L t_j} + c_3\|Y_{\xi^-}\|\right)\;\mathrm{d}s\;\mathrm{d}\xi.
    \end{multline*}
    ...
    \[
        \left\|\int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right\| \leq \tilde C_N \Delta t,
    \]
    where $\tilde C_N$ is the random variable
    \begin{multline}
        \tilde C_N = \int_0^T \left(c_1 + c_2 \|X_{\tau^N(s)}^N\| + c_3\|Y_{s^-}\|\right)\;\mathrm{d}s \\
