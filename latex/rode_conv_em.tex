...
In this section, we illustrate the strong order 1 convergence with a few explicit examples that fall into one of the cases considered above.
...
Here are the main variables in the examples:
\begin{enumerate}
    ...
    \item A number $N_{\mathrm{fine}}$ of mesh points for a fine discretization to compute a target solution path, typicall $N_{\mathrm{fine}} = \max_i{N_i^2}$, e.g. $N=2^{20} = 1,048,576$;
    \item The target solution path is either an exact pathwise solution, when available, or a (higher-order) approximation.
\end{enumerate}

For each sample $m=1, \ldots, M$, we first generate a discretization $\{Y_{t_j}\}_{j=0, N_{\mathrm{fine}}}$ of a sample path of the noise on the finest grid with $N_{\mathrm{fine}}$ points. Next we use these values to construct the target solution $\{X_{t_j}\}_{j=0, N_{\mathrm{fine}}}$ still on the fine mesh. This is obtained either using the Euler approximation itself, keeping in mind that the time mesh is sufficiently fine, or by an exact distributions of the solution, when available. Next, for each time step $N_i = 2^{n_i}$ in the selected range, we compute the Euler approximation using the noise values at the corresponding coarse mesh. We compare that to the values of the target path on that coarse mesh and update the strong error
\[
    \frac{1}{M}\sum_{m=1}^M \left|X_{t_j}(\omega_m) - X_{t_j}^{N_i}(\omega_m)\right|
\]
at each mesh point. At the end, we take the maximum in time, for each coarse mesh to obtain the error for each mesh
\[
    \epsilon_i = \max_{j=0, \ldots, N_i} \frac{1}{M}\sum_{m=1}^M \left|X_{t_j}(\omega_m) - X_{t_j}^{N_i}(\omega_m)\right|
\]

Finally, we fit $(\Delta_i, \epsilon_i)$ to $C\Delta_i^p$, for suitable $C$ and $p$, with $p$ giving the order of convergence.
