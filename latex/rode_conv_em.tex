...
\begin{enumerate}
    ...
    \item An Ornstein-Uhlenbeck process with drift $\nu = 0.3,$ diffusion $\sigma = 0.2,$ and initial condition $y_0 = 0.2$;
    \item A geometric Brownian motion process with drift $\mu = 0.3,$ diffusion coefficient $\sigma = 0.3,$ and initial condition $y_0 = 0.2$;
    \item A compound Poisson process with rate $\lambda = 2$ and jump law following a Normal distribution with mean $\mu = 0.3$ and standard deviation $\sigma = 0.3$;
    \item A Poisson step process with rate $\lambda = 10.0$ and step law following a Beta distribution with shape parameters $\alpha = 2.0$ and $\beta = 15.0;$
    \item An exponentially decaying Hawkes process with initial rate $\lambda_0 = 2.0$, base rate $a = 0.8$, exponential decay rate $\delta = 0.9$, and jump law following an exponential distribution with scale $\beta = 1.8,$ corresponding to a rate $\lambda = 1/1.8 \approx 0.56;$
    \item A transport process of the form $t \mapsto \sum_{i=1}^{20} \sin^{1/3}(t/r_i)$, where $r_i$ are independent random variables following a Beta distribution with shape parameters $\alpha = 2.0$ and $\beta = 15.0;$
...
\end{equation}
computing an distributionally exact solution of this form is a delicate process. Thus we check the convergence numerically by solving the equation with the Euler method itself, but on a much finer mesh. Nevertheless, the proof is done rigorously below, with the framework developed in the first sections.
