...
\begin{proof}
    ...
    For the second term, we use the Bichteler-Dellacherie Theorem \cite[Theorem 47]{Protter2005} which says that $\{Y_t\}_{t\in I}$, as any semi-martingale, can be decomposable in the form $Y_t = F_t + Z_t$, where $\{F_t\}_{t\in I}$ is an FV process (almost surely c\`adl\`ag paths of finite variation) and $\{Z_t\}_{t\in I}$ is a local martingale. Hence, the second term can be written as
    \begin{multline}
        \label{sm2}
    ...
    \end{multline}
    where the first inner integral is a Lebesgue-Stieltjes integral and the second inner integral is a stochastic integral. Since $\{F_t\}_{t\in I}$ is of finite variation, the first part of \eqref{sm2} is estimated as in the proof of \cref{thmcadlagfv}, so that
    \begin{align*}
