...
\thanks{The second author was partly supported by  Laborat\'orio de Matem\'atica Aplicada, Instituto de Matem\'atica, Universidade Federal do Rio de Janeiro (LabMA/IM/UFRJ)}
...
Here we derive an estimate that is the basis for the specific estimates for each type of noise. For that, we use the following discrete version of the Grownwall Lemma, which is a particular case of the result found in \cite{GiraultRaviart1981} (see also \cite{Clark1987}).
...
This follows from \cite[Lemma V.2.4]{GiraultRaviart1981} by taking $n = j$, $a_n = e_j$, $b_n = 0$, $c_n = b$, and $\lambda = a$.
%\begin{proof}
...
First we simulate a linear equation with several types of noises, emcompassing noises with sample paths with bounded variation and It\^o process noises. Next we simulate a linear equation with fBm noise. Then we implement a couple nonlinear problems. Further examples can be found in the github repository \cite{github}, such as a Here, we consider a population dynamics modelled by a logistic equation with random coefficients, loosely inspired by \cite[Section 15.2]{HanKloeden2017}, with an extra term representing harvest; and a Fisher-KPP partial differential equation \cite{Fisher1937} and \cite{KPP1937} with random boundary conditions, as inspired by the works of \cite{SalakoShen2020}  and \cite{FreidlinWentzell1992}.
...
\begin{figure}[htb]
    ...
    \caption{Order of convergence of the strong error of the Euler method for the toggle-switch model for gene expression \eqref{toogleswitchsystem}.}
