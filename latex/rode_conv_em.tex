...
\title[Convergence of the Euler method for Random ODEs with semi-martingales]{Improved order of convergence of the Euler method for random ordinary differential equations driven by semi-martingales}
...
\thanks{The second author was partly supported by the Laborat\'orio de Matem\'atica Aplicada, Instituto de Matem\'atica, Universidade Federal do Rio de Janeiro (LabMA/IM/UFRJ) and the Coordenação de Aperfeiçoamento de Pessoal de Nível Superior (CAPES), Brasil (CAPES), finance code 001}
...
For the theory of semi-martingales, we refer the reader to \cite{Protter2005}. Thanks to the Bichteler-Dellacherie Theorem \cite[Theorem III.47]{Protter2005}, a semi-martingale is a process $\{Y_t\}_{t\in I}$ characterized by being decomposable as a sum $Y_t = F_t + Z_t$ of an FV process $\{F_t\}_{t\in I}$ and a local martingale process $\{Z_t\}_{t\in I}$.

An FV process, or Finite-Variation process, is a stochastic process whose sample paths are almost surely c\`adl\`ag paths of finite variation. A function defined on a real interval is called c\`adl\`ag when it is right continuous and with left limits (``continue \`a droite, limite \`a gauche", in French). Thus, an FV process $\{Y_t\}_{t\in I}$ is such that, for almost every $\omega\in \Omega$ and for all $t\in I$, we have that $\lim_{s\rightarrow t^+} Y_s(\omega) = Y_t(\omega)$ (right continuous) and the limit $\lim_{s \rightarrow t^-} Y_s(\omega)$ exists (left limit), and the path is of finite variation $V(\{Y_t\}_{t\in I}; I)$, also called bounded variation, i.e.
\[ V(\{Y_t\}_{t\in I}; I) = \sup_{t_0 < \ldots < t_n \in I, n\in \mathbb{N}} \sum_{i=1}^n \|Y_{t_i} - Y_{t_{i-1}}\| < \infty.
\]

For such a process, we define the left-limit process
\begin{equation}
...
\begin{equation}
  \Delta Y_t = Y_t - Y_{t^{-}}.
\end{equation}
Almost surely, the sample paths of the jump process are zero except possibly at a countable number of points, denoted $J$, i.e. for almost every $\omega\in \Omega$, there exists a countable set $J(\omega) \subset I$ for which $\Delta Y_t(\omega)$ is zero on each connected component of $I\setminus J(\omega)$, and such that
\[
...
Given a continuously differentiable function $f:\mathbb{R}^k \rightarrow \mathbb{R}^k$, the following change of variables formula holds for such processes (see \cite[Theorems II.31 and II.33]{Protter2005}):
\begin{equation}
...
The class of local martingales include It\^o diffusion processes and time-changed Brownian motion, for example.
...
Here, the noise $\{Y_t\}_{t\in I}$ is assumed to have sample paths that are c\`adl\`ag and with finite variation.
...
\begin{proof}
    ...
    For the second term in \eqref{smlasttermerror}, we use the Bichteler-Dellacherie Theorem \cite[Theorem III.47]{Protter2005}, which says that $\{Y_t\}_{t\in I}$, as any semi-martingale, can be decomposed as $Y_t = F_t + Z_t$, where $\{F_t\}_{t\in I}$ is an FV process (almost surely c\`adl\`ag paths of finite variation) and $\{Z_t\}_{t\in I}$ is a local martingale. Hence, the second term can be written as
    \begin{multline}
        \label{sm2}
...
\]
where $\gamma > 0,$ $0 < \varepsilon < 1,$ and $\{G_t\}_{t\geq 0}$ is a geometric Brownian motion (gBm) process. A Wiener process is a natural choice, but we choose a gBm process instead since it is a multiplicative noise, thus the convergence would not be expected to be of strong order 1 based on previous works.
