...
\begin{align*}
    f(s, B^H_{s, t}) & = f(0, B^H_{0, t}) + \int_{0}^s \frac{\partial f}{\partial s}(t, B_{\tau, t})\;\mathrm{d}\tau + \alpha \int_{0}^s \frac{\partial f}{\partial y}(t, B_{\tau, t})\tau^{\alpha - 1} (Y^H_{\tau, t} - Y^H_{\tau, \tau})\;\mathrm{d}\tau \\
    & \quad + \frac{C_H^2}{2}\int_{0}^s \frac{\partial^2 f}{\partial y^2}(t, B_{\tau, t}) (t - \tau)^{2\alpha} \;\mathrm{d}s + C_H \int_{0}^s \frac{\partial f}{\partial y}(t, B_{\tau, t})(t - \tau)^\alpha\;\mathrm{d} B_\tau
\end{align*}

Thus, using that the partial derivatives are continuous and uniformly bounded,
\begin{align*}
    \mathbb{E}\left[\left|f(s, B^H_{s, t}) - f(0, B^H_{0, t})\right|\right] & \leq s + \int_{0}^s \tau^{\alpha-1}\left(\mathbb{E}\left[\left|Y^H_{\tau, t}\right|\right] + \mathbb{E}\left[\left|Y^H_{\tau, \tau}\right|\right] \right)\;\mathrm{d}\tau \\
    & \quad + \frac{C_H^2}{2}\int_{0}^s (t - \tau)^{2\alpha} \;\mathrm{d}s + C_H \left(\int_{0}^s (t - \tau)^{2\alpha}\;\mathrm{d} \tau\right)^{1/2}.
\end{align*}
