...
\begin{lemma}
    ...
    Under the \cref{standinghypotheses1}, assume that \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound} hold. Then
    \begin{equation}
...
\begin{align*}
    ...
    & \leq \|X_{t_{j-1}}^N\| + \Delta t_N (M(\|Y_{t_{j-1}}\|) + L(\|Y_{t_{j-1}}\|)\|X_{t_{j-1}}\|) \\
    & \leq \left(1 + L(\|Y_{t_{j-1}}\|)\Delta t_N\right)\|X_{t_{j-1}}^N\| + \Delta t_N M(\|Y_{t_{j-1}}\|).
\end{align*}
We bound $(1 + a) \leq e^a$, so that
\[
    \|X_{t_j}^N\| \leq e^{L(\|Y_{t_{j-1}}\|)\Delta t_N}\|X_{t_{j-1}}^N\| + \Delta t_N M(\|Y_{t_{j-1}}\|).
\]
Iterating this relation we obtain the following bound.
\begin{equation}
    \label{XNtboundLXMtpre}
    \|X_{t_j}^N\| \leq \left(\|X_0^N\| + \Delta t_N \sum_{i=0}^{j-1} M(\|Y_{t_i}\|)\right)e^{\Delta t_N \sum_{i=0}^{j-1} L(\|Y_{t_i}\|)}, \quad \forall j = 0, \ldots, N.
\end{equation}
...
\begin{lemma}
    ...
    \begin{equation}
        \label{XNtboundLXMt}
        \|X_{t_j}^N\| \leq \left(\|X_0^N\| + t_j \sup_{0\leq s\leq t_j} M(\|Y_s\|)\right)e^{t_j \sup_{0\leq s\leq t_j} L(\|Y_s\|)}, \quad \forall j = 0, \ldots, N.
    \end{equation}
    \qed
\end{lemma}
...
\begin{lemma}
    ...
    Under the \cref{standinghypotheses1}, assume moreover that \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound} hold. Then, the Euler approximation \eqref{emscheme} for a pathwise solution of the random ordinary differential equation \eqref{rodeeq} satisfies
    \begin{equation}
        \label{XNtmeanboundLXMt}
        \mathbb{E}\left[\|X_{t_j}^N\|\right] \leq \left(\mathbb{E}\left[\|X_0^N\|\right] + \mathbb{E}\left[\sum_{i=0}^{j-1} M(\|Y_{t_i}\|)\Delta t_N \right] \right)e^{c_L t_j}, \quad \forall j = 0, \ldots, N.
    \end{equation}
    \qed
\end{lemma}
...
\begin{lemma}
    ...
    \begin{multline}
        ...
        \leq \Delta t_N \sup_{0 \leq s \leq t_j}L(\|Y_s\|) \int_0^{t_j} \left(M(\|Y_s\|) + L(\|Y_s\|)\|X_s\|\right) \;\mathrm{d}s.
    \end{multline}
\end{lemma}
...
\begin{proof}
    ...
    \[
      \left\|X_s - X_{\tau^N(s)}\right\| = \left\|\int_{\tau^N(s)}^s f(\xi, X_\xi, Y_\xi)\;\mathrm{d}\xi\right\| \leq \int_{\tau^N(s)}^s \left(M(\|Y_\xi\|) + L(\|Y_\xi\|)\|X_\xi\|\right)\;\mathrm{d}\xi.
    \]
    Integrating over $[0, t_j]$ and using Fubini's theorem to exchange the order of integration,
    \begin{multline*}
        \int_0^{t_j}L(\|Y_s\|) \left\|X_s - X_{\tau^N(s)}\right\| \;\mathrm{d}s \leq \int_0^{t_j}L_s\int_{\tau^N(s)}^s (M(\|Y_\xi\|) + L(\|Y_\xi\|)\|X_\xi\|) \;\mathrm{d}\xi \;\mathrm{d}s \\
        = \int_0^{t_j}(M(\|Y_\xi\|) + L(\|Y_\xi\|)\|X_\xi\|) \int_\xi^{\tau^N(\xi) + \Delta t_N} L(\|Y_s\|)\;\mathrm{d}s \;\mathrm{d}\xi.
    \end{multline*}
    ...
    \begin{multline*}
        ...
        \leq \int_0^{t_j} (\tau^N(\xi) + \Delta t_N - \xi) \sup_{0\leq s \leq t_j}L(\|Y_s\|)(M(\|Y_\xi\|) + L(\|Y_\xi\|)\|X_\xi\|) \;\mathrm{d}\xi
    \end{multline*}
...
\begin{lemma}
    Under the \cref{standinghypotheses1} and assuming conditions \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound}, we find that
    \begin{multline}
        ...
        \leq \Delta t_N c_L \left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M(\|Y_s\|)]\;\mathrm{d}s\right) e^{c_L t_j}.
    \end{multline}
\end{lemma}
...
\begin{proof}    
    ...
    \[
        \mathbb{E}\left[\int_0^{t_j}L(\|Y_s\|)\left\|X_s - X_{\tau^N(s)}\right\| \;\mathrm{d}s\right] \leq \Delta t_N c_L \int_0^{t_j} \left(\mathbb{E}\left[M(\|Y_s\|)\right] + c_L\mathbb{E}\left[\|X_s\|\right]\right) \;\mathrm{d}s.
    \]
    Thanks to \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound}, \cref{lemstrongbound} applies and inequality \eqref{EXtstrongbound} holds, so that we obtain
    \begin{align*}
        \mathbb{E}\bigg[\int_0^{t_j} & L(\|Y_s\|) \|X_s - X_{\tau^N(s)}\| \;\mathrm{d}s\bigg] \\
        & \leq \Delta t_N c_L \int_0^{t_j} \bigg(\mathbb{E}[M(\|Y_s\|)] + c_L\left(\mathbb{E}[\|X_0\|] + \int_0^s \mathbb{E}[M(\|Y_\xi\|)]\;\mathrm{d}\xi\right)e^{c_L s} \bigg)\;\mathrm{d}s \\
        & = \Delta t_N c_L \bigg(\int_0^{t_j} \mathbb{E}[M(\|Y_s\|)] \;\mathrm{d}s \\
        & \hspace{1in} + c_L\int_0^{t_j}\left(\mathbb{E}[\|X_0\|] + \int_0^s \mathbb{E}[M(\|Y_\xi\|)]\;\mathrm{d}\xi\right)e^{c_L s} \;\mathrm{d}s\bigg) \\
        & = \Delta t_N c_L \bigg(\int_0^{t_j} \mathbb{E}[M(\|Y_s\|)] \;\mathrm{d}s \\
        & \hspace{1in} + \left(\mathbb{E}[\|X_0\|] + \int_0^{t_j} \mathbb{E}[M(\|Y_\xi\|)]\;\mathrm{d}\xi\right)\left(e^{c_L t_j} - 1\right) \bigg).
    \end{align*}
...
\begin{theorem}
    ...
    Under the \cref{standinghypotheses1}, suppose further that \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound} hold; that $X_0^N$ is a first order approximation of $X_0,$ in the sense that 
    \begin{equation}
        \label{EX0X0Nunif}
...
\begin{theorem}
    ...
    Under the \cref{standinghypotheses1}, suppose further that \eqref{LtLXbound}, \eqref{CMmeanbound}, and \eqref{EX0strongbound} hold; that the initial condition satisfies
    \begin{equation}
...
\begin{thebibliography}{25}
    ...
    \bibitem{GerberShiu1998} H. U. Gerber, E. S. W. Shiu, On the time value of ruin, North American Actuarial J., vol. 2 (1998), no. 1, 48--72.
    ...
    \bibitem{Metivier1982} M. M\'etivier, Semimartingales: A Course on Stochastic Processes, Volume 2 in the series De Gruyter Studies in Mathematics, De Gruyter, Berlin, New York, 1982.
    ...
    \bibitem{Sugiyama1969} S. Sugiyama, Stability problems on difference and functional-differential equations, \emph{Proc. Japan Acad.} 45 (1969), no. 7, 526--529. % DOI: 10.3792/pja/1195520661
