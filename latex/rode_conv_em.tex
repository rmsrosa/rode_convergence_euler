...
\title[Strong order 1 convergence of Euler for Random ODEs]{Strong order 1 convergence of the Euler method for Random Ordinary Differential Equations}
...
In this section, we describe a few explicit examples that fall into one of the cases considered above and, hence, the Euler method exhibits a strong order one convergence.

For estimating the order of convergence, we use the Monte Carlo method, computing a number of numerical approximations $\{X_{t_j}^N(\omega_m)\}_{j=0, \ldots, N}$, of sample path solutions $\{X_t(\omega_m)\}_{t\in I}$, for $\omega_1, \ldots, \omega_M$ and taking the maximum in time of the average of their absolute differences at the mesh points:
\begin{equation}
    \textrm{strong error } = \max_{j=0, \ldots, n} \frac{1}{M}\sum_{m=1}^M \left|X_{t_j}(\omega_m) - X_{t_j}^N(\omega_m)\right|
\end{equation}

We follow two approaches, depending on whether we have an explicit formula for the exact solutions or not. In the first case, we first compute a numerical approximation, which includes computing the noise term at the mesh points, $\{Y_{t_j}\}_{j=0, \ldots, n}$, and then we use this points to compute the exact solution.

In the second case, we first compute the sample paths of the noise on a very fine time mesh and then we use this to compute the numerical approximation from the values at the coarse mesh and also to compute an approximation of the exact solution from the fine mesh.
...
\end{equation}
We cannot compute the integral $\int_0^{t_j} W_s\;\mathrm{d}s$ exactly just from the values $W_{t_j}$ of the noise on the mesh points, but we can get a more explicit formula for it.
...
\end{equation}
is a Brownian bridge on the interval $[t_i, t_{i+1}]$, independent of $\{W_t\}_{t\geq 0}$. Then,
\begin{align*}
