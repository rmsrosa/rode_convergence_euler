...
The spatial discretization is done with finite differences on a grid with the number of mesh points varing from 17 to 65 mesh points, depending on the time mesh. For the time-mesh parameters, we set $N_{\textrm{tgt}} = 2^{18}$ and $N_i = 2^5, 2^7, 2^9$. The spatial mesh has 17 points when $N_i=2^5$, 33 when $N_i=2^7$ and 65 when $N_i=2^9$ and for $N_{\textrm{tgt}} = 2^18$.

For the Monte-Carlo estimate of the strong error, we choose $m=200.$ Table \ref{tablefisherkpp} shows the estimated strong error obtained with this setup, while \cref{figfisherkpp} illustrates the order of convergence.

\begin{table}
    \begin{tabular}[htb]{|r|l|l|l|}
        \hline N & dt & error & std err \\
        \hline \hline
        1024 & 0.00196 & 0.244 & 0.0155 \\
        1152 & 0.00174 & 0.215 & 0.0136 \\
        1280 & 0.00156 & 0.195 & 0.0124 \\
        1536 & 0.0013 & 0.163 & 0.0103 \\
        1920 & 0.00104 & 0.131 & 0.00829 \\
        2048 & 0.000977 & 0.124 & 0.00782 \\
        \hline
    \end{tabular}
    \bigskip

    \caption{Mesh points (N), time steps (dt), strong error (error), and standard error (std err) of the Euler method for Fisher-KPP equation for each mesh resolution $N$, with initial condition $X_0 = 0$ and Hawkes-modulated colored noise, on the time interval $I = [0.0, 2.0]$, based on $M = 200$ sample paths for each fixed time step, with the target solution calculated with $4423680$ points. The order of strong convergence is estimated to be $p = 0.981$, with the 95\% confidence interval $[0.7565, 1.2047]$}.
