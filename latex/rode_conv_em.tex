...
\begin{figure}[htb]
    ...
    \caption{Order of convergence $p = 0.974$ of the strong error of the Euler method for
    $\mathrm{d}X_t/\mathrm{d}t = W_t X_t$, on $[0, T] = [0.0, 1.0]$, with $X_0 \sim \mathcal{N}(0, 1)$,
    and a standard Wiener process noise $\{W_t\}_t$,
    computed with $M = 500$ sample paths,
    for each $\Delta t = 1/N$,  $N = 2^n$, $n=4, \ldots, 14.$, with $N_{\textrm{tgt}} = 2^{16}$.}
...
\begin{figure}[htb]
    ...
    \caption{Order of convergence $p = 1.016$ (95\% CI[0.9418, 1.0897]) of the strong error of the Euler method for $\mathrm{d}\mathbf{X}_t/\mathrm{d}t = - \left\|\mathbf{Y}_t\right\|^2 \mathbf{X}_t + \mathbf{Y}_t,$ based on \cref{taballnoises}.}
    \label{figallnoises}
\end{figure}
