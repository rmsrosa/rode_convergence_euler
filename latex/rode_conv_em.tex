...
Each term involves an inner deterministic integral which can be readily computed. For the first term, using that $\sigma \mapsto 1/(\sigma - \xi)^{H-1/2}$ is continuously differentiable on the interval $\sigma > \xi$, using the Fundamental Theorem of Calculus and exchanging the order of integration, we find
\begin{multline*}
...
The second term is similar and yields
\begin{multline*}
