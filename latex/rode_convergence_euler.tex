...
For the theory of semi-martingales, we refer the reader to \cite{Protter2005,Metivier1982,AitSahaliaJacod2014,KaratzasShreve2014,HeWangYan1992}. The main idea is that \emph{semi-martingales} are stochastic processes $\{Y_t\}_{t\geq 0}$ which are c\`adl\`ag, adapted, and ``good integrators,'' in the sense that the sums
\[
...
\begin{equation}
    ...
    \sum_{s\in J_{Y}} \|\Delta F_s\| \leq V(\{F_t\}_t; I) < \infty.
\end{equation}
...
\begin{equation}
    ...
    \mathbb{E}\left[\sum_{s\in J_{Y}} \|\Delta Y_s\|^2 \right] \leq 4\gamma_2\mathbb{E}\left[\|Z_T\|^2\right] + \mathbb{E}\left[\|V(\{F_t\}_t; I)\|^2\right].
\end{equation}
...
\begin{equation}
    ...
    \mathbb{E}\left[V(\{[Z, Z]_t^c\}; I)\right] \leq 5\gamma_2\mathbb{E}\left[\|Z_T\|^2\right] < \infty.
\end{equation} 
...
Finally, a fundamental identity is the It\^o isometry, which, in the case of a vector-valued local martingale $\{Z_t\}_{t\geq 0}$ and an adapted matrix-valued process $\{A_t\}_{t\geq 0},$ takes the form (e.g. apply \cite[Chapter 3, Proposition 2.17]{KaratzasShreve2014} to the expanded coordinate terms)
\begin{multline}
...
\end{multline}
provided the right hand side is finite, and where $\odot$ denotes the Hadamard product of two matrices of same dimensions. Recall that the steps of the cross variation matrix $[Z, Z]_s$ are positive semi-definite, so the right hand side is nonnegative.
...
\begin{proof}
    Using that the paths of $\{X_t\}_t$ are almost surely absolutely continuous, adding and subtracting $f(\xi, 0, Y_\xi)$ to the integrand, and using assumption \eqref{standinghypothesisLipschitzbound} of the \cref{standinghypotheses1} along with \cref{LtLXbound}, we have almost surely that
    \[
...
\begin{theorem}
    ...
    \end{align}
    in $(t, x, y)\in I\times \mathbb{R}^d\times \mathbb{R}^k$, for suitable constants $c_1, c_2, c_3, c_4, c_7 \geq 0$. Consider the semi-martingale noise $\{Y_t\}_{t\in I}$ decomposed as $Y_t = Y_0 + F_t + Z_t$, where $\{F_t\}_{t\in I}$ is an FV process, $\{Z_t\}_{t\in I}$ is a local martingale, and $F_0 = Z_0 = 0,$ and assume that $\{Z_{t \wedge T}\}_{t\geq 0}$ is a martingale, and they satisfy
    \begin{align}
        \label{Y0finiteexpectation}
        & \mathbb{E}\left[\|Y_0\|\right] < \infty, \\
        \label{expectFVFtfinite}
        & \mathbb{E}\left[V(\{F_t\}_t; I)^2\right] < \infty, \\
        \label{Zmartingale}
...
The authors thank Glauco Valle (IM/UFRJ), for helpful discussions about semi-martingales, and Luan Lima Freitas (IM/UFRJ), for helpful comments on the manuscript. The second author was partly supported by Laborat\'orio de Matem\'atica Aplicada, Instituto de Matem\'atica, Universidade Federal do Rio de Janeiro (LabMA/ IM/UFRJ), Coordena\c{c}\~ao de Aperfei\c{c}oamento de Pessoal de N\'{\i}vel Superior (CAPES), Brasil, grant 001, and Conselho Nacional de Desenvolvimento Cient\'{\i}fico e Tecnol\'ogico (CNPq), Bras\'{\i}lia, Brasil, grant no. 408751/2023-1.
