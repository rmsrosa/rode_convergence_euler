...
For the estimates using the It\^o isometry \cref{eqitoisometrylocalmartingale}, it is important to note that the polarization identity
\[ [Y^{(l_1)}, Y^{(l_2)}] = \frac{1}{4}\left([Y^{(l_1)} + Y^{(l_2)}, Y^{(l_1)} + Y^{(l_2)}] - [Y^{(l_1)} - Y^{(l_2)}, Y^{(l_1)} - Y^{(l_2)}]\right)
\]
implies that each quadratic covariation is a difference of quadratic variations, which are non-decreasing processes, so that each quadratic covariation is also of finite variation, with
\begin{equation}
    \label{eqquadraticvarcovarfiniteexpectation}
    \sum_{l_1, l_2=1}^k \mathbb{E}\left[V(\{[Z^{(l_1)}, Z^{(l_2)}]_t\}; I)\right] < \infty.
\end{equation} 
