...
\begin{proof}
    ...
    \end{align*}
    \comment{Ops, the term $\left|\mathrm{d}[Z^{(l_1)}, Z^{(l_2)}]_\xi\right|$ should be squared, or just use $\ell^1$ norm in $\mathbb{R}^n$ and then go to the equivalent $\ell^2$}
    ...
    The final term in \cref{smlasttermerror} involves a Lebesgue-Stieltjes integral and is handled in a similar way, with
    \begin{align*}
    ...
    \comment{Ops, it remains to consider the vector-valued form of this, where $[Y, Y]_{t_j}$ is a matrix, etc., so we should actually have}
    \begin{align*}
        \bigg\|\frac{1}{2} \int_0^{t_j} \int_{\tau^N(s)}^s & D_{yy}f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\;\mathrm{d}[Y, Y]_\xi^c\;\mathrm{d}s \bigg\|^2 \\
        & = \left(\frac{1}{2} \sum_{i=1}^n\int_0^{t_j} \int_{\tau^N(s)}^s \left(D_{yy}f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-})\;\mathrm{d}[Y, Y]_\xi^c\right)^{(i)}\;\mathrm{d}s \right)^2\\
        & = \left(\frac{1}{2} \sum_{i=1}^n\int_0^{t_j} \int_{\tau^N(s)}^s \sum_{l_1, l_2 = 1, \ldots, d} \partial_{y_{l_1}}\partial_{y_{l_2}} f_i(s, Y_{s^-})\;\mathrm{d}[Y^{(l_1)}, Y^{(l_2)}]_s^c\;\mathrm{d}s\right)^2 \\
        & \leq \ldots
    \end{align*}
    \comment{Ops, again, careful with the $\ell^1$ / $\ell^2$ norms}
