...
\]
In the vector-valued case, denoting $Y_t = (Y_t^{(l)})_{l=1, \ldots, k}$, the quadratic process becomes matrix-valued, i.e. $[Y, Y]_t = ([Y^{(l_1)}, Y^{(l_2)}]_t)_{l_1,l_2=1,\ldots, k},$ with the appearence of the quadratic cross-variations
\[
...
\[ [Y^{(l_1)}, Y^{(l_2)}] = \frac{1}{4}\left([Y^{(l_1)} + Y^{(l_2)}, Y^{(l_1)} + Y^{(l_2)}] - [Y^{(l_1)} - Y^{(l_2)}, Y^{(l_1)} - Y^{(l_2)}]\right)
\]
implies that each quadratic cross-variation is a difference of quadratic variations, which are non-decreasing processes, so that each quadratic cross-variation is of finite variation.
...
\begin{multline*}
    \mathbb{E}\left[\left\|\int_0^t A_s \;\mathrm{d}Z_s\right\|^2\right] = \mathbb{E}\left[ \int_0^t A_s^\tr A_s \odot \mathrm{d}[Z_s, Z_s]\right] \\ = \mathbb{E}\left[ \int_0^t \sum_{l_1, l_2 = 1}^k \sum_{i=1}^d A_s^{(il_1)}A_s^{(il_2)}\;\mathrm{d}[Z_s^{(l_1)}, Z_s^{(l_2)}]\right],
\end{multline*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        & \leq c_4\Delta t_N\mathbb{E}\left[\sum_{l_1,l_2=1}^k\int_0^{t_j} \;\mathrm{d}[Z^{(l_1)}, Z^{(l_2)}]_\xi\right]^{1/2} \\
        & = c_4\Delta t_N\mathbb{E}\left[\sum_{l_1,l_2=1}^k[Z^{(l_1)}, Z^{(l_2)}]_{t_j} - [Z^{(l_1)}, Z^{(l_2)}]_0\right]^{1/2},
    \end{align*}
    ...
    \begin{equation}
        ...
        \mathbb{E}\bigg[\bigg\|\int_0^{t_j} \int_{\tau^N(s)^+}^s D_y f(\xi, X_{\tau^N(s)}^N, Y_{\xi^-}) \;\mathrm{d}Z_\xi\;\mathrm{d}s\bigg\|\bigg] \leq c_4\sqrt{k}\Delta t_N\mathbb{E}\left[\left\| [Z, Z]_{t_j}\right\|\right]^{1/2}.
    \end{equation}
