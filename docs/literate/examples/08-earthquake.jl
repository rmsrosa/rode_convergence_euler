...
#
# The structure is originally at rest, before the first groundshaking excitation, so we have the conditions
#
# ```math
...
#
# With this in mind, we model the background colored noise with an Orsntein-Uhlenbeck (OU) process $\{O_t\}_t$ with a moderately small time scale $\tau$. More precisely, we assume $\{O_t\}_t$ satisfies the stochastic differential equation
#
# ```math
...
yt = yt2 .* yt1
dt = (tf - t0) / (ntgt - 1)
ground = accumulate(+, accumulate(+, map(prod, eachrow(yt)))) * dt ^ 2

tt = range(t0, tf, length=length(yt))
plot(tt, ground, title="ground displacement")
