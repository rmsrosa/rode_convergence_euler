...
```
is substituted by a randomly varying parameter given as a stochastic process $\{M_t\}$ representing a base growth parameter $\mu$ "perturbed" by a white noise, i.e.
```math
M_t = \mu + \sigma\frac{\mathrm{d}W_t}{\mathrm{d}t},
```
for some perturbance *amplitude* $\sigma > 0.$ This means rigorously that
```math
...
More generally, the basal coefficient $\mu$ and the perturbation amplitude $\sigma$ in the equation for $M_t$ may also vary deterministically for seasonal reasons, on top of the random disturbances, so it is natural to consider the non-autonomous version
```math
\mathrm{d}M_t = a(t)\;\mathrm{d}t + b(t)\mathrm{d}W_t.
```

In this case, the gBm equation is substituted by a more general linear homogeneous equation with time-dependent coefficients,
```math
...
```
with $Y_0 = y_0$, and where $p_a = p_a(t)$ is the given primitive of $a=a(t),$ $p_{b^2} = p_{b^2}(t)$ is the given primitive of $b^2 = b(t)^2,$ and $\{Z_j\}_j$ is a random vector with independent nornally distributed variables
```math
...
nothing # hide
```
or via the homogeneous linear Itô process constructor, by given the primitive of the constants $\mu$ and $\sigma^2$:
```@example homlin
primitive_a = t -> μ * t
primitive_b2 = t -> σ^2 * t
noise = HomogeneousLinearItoProcess(t0, tf, y0, primitive_a, primitive_b2)
```
With this setup, we create the corresponding process:
...
We consider the following primitives of $a(t) = -2t$ and $b(t)^2 = \sin(3\pi t)^2:$
```math
