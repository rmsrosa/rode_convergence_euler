"""
    RandomRalston(T::DataType=Float64, n::Int=0)

Instantiate a `RandomRalston` method including three cache vectors of length `n` for a non-allocating solver via the Ralston method, solved by `solve!(xt, t0, tf, x0, f, yt, params, ::RandomRalston))`.
...
"""
struct RandomRalston{T, N} <: RODEMethod{N}
    cachex1::Vector{T}
    cachef1::Vector{T}
    cachef::Vector{T}
    function RandomRalston(T::DataType, n::Int)
        n ≥ 0 || error(
...
RandomRalston(T::DataType) = RandomRalston(T, 0)
RandomRalston(n::Int) = RandomRalston(Float64, n)
RandomRalston() = RandomRalston(Float64, 0)

function solve!(xt::AbstractVector{T}, t0::T, tf::T, x0::T, f::F, yt::AbstractVector{T}, params::Q, ::RandomRalston{T, Univariate}) where {T, F, Q}
    ...
    for i in Iterators.drop(eachindex(xt, yt), 1)
        ...
        xtnaux = xt[i1] + dt * fn1
        ti1 += dt
        xt[i] = xt[i1] + dt * ((1/3)fn + (2/3)f(ti1 + (3/4)dt, xtnaux, yt[i] + (3/4)dt * fn1, params)) / 2
        i1 = i
    end
end

function solve!(xt::AbstractVector{T}, t0::T, tf::T, x0::T, f::F, yt::AbstractMatrix{T}, params::Q, ::RandomRalston{T, Univariate}) where {T, F, Q}
...
function solve!(xt::AbstractMatrix{T}, t0::T, tf::T, x0::AbstractVector{T}, f::F, yt::AbstractVector{T}, params::Q, method::RandomRalston{T, Multivariate}) where {T, F, Q}
...
function solve!(xt::AbstractMatrix{T}, t0::T, tf::T, x0::AbstractVector{T}, f::F, yt::AbstractMatrix{T}, params::Q, method::RandomRalston{T, Multivariate}) where {T, F, Q}
