...
\begin{lem}
    ...
    \begin{equation}
        \label{stepbound}
          |f(s, X_\tau, Y_s) - f(\tau, X_\tau, Y_\tau)| \leq \bar F_s - \bar F_\tau,
      \end{equation}
...
\begin{cor}
    ...
    Suppose that $f=f(t, x, y)$ is uniformly globally Lipschitz continuous in $x$ and is continuously differentiable in $(t, y)$ with partial derivatives $\partial_t f$ and $\partial_y f$ with at most linear growth in $x$, i.e.
    \begin{equation}
        \label{ftfylineargrowth}
        \left|\frac{\partial f}{\partial t}(t, x, y)\right| \leq C_1 + C_2 |x|, \quad \left|\frac{\partial f}{\partial t}(t, x, y)\right| \leq C_3 + C_4 |x|,
    \end{equation}
    in $(t, x, y)\in I\times \mathbb{R}\times \mathbb{R}$,
    for suitable constants $C_1, C_2, C_3, C_4 \geq 0$.
    Assume, further, that the sample paths of $\{Y_t\}_{t\in I}$ are of bounded variation $V(\{Y_t\}_{t\in I}; I)$, on $I$, with finite quadratic mean variation
    \begin{equation}
...
\begin{lem}
    \label{lemItostep}
    Besides the \cref{standinghypotheses1}, suppose that $F_t^N = f(t, X_{\tau^N(t)}, Y_t)$ is an It\^o noise, satisfying
    \begin{equation}
        \label{Itostep}
