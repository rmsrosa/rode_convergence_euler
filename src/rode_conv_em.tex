...
\begin{hyp}[Standing Hypotheses I]
    ...
    \begin{enumerate}
        \item $f$ is globally Lipschitz continuous on $x$, uniformly in $t$ and $y$, i.e. there exists $L_x > 0$ such that
            \begin{equation}
                \label{Lxassumptionbasic}
                |f(t, x_1, y) - f(t, x_2, y)| \leq L_x |x_1 - x_2|, \qquad \forall t \in [0, T], \;\forall x_1, x_2, y\in\mathbb{R}.
            \end{equation}

        \item We also assume that $(t, x) \mapsto f(t, x, Y_t)$ satisfies the \emph{Carath\'eodory conditions.} More precisely, we assume that
            \begin{enumerate}
                ...
                \item The bound $|f(t, x, Y_t)| \leq M_t^{(0)} + M_t^{(1)}|x|$ holds for all $t\in I$ and all $x\in\mathbb{R}$, where $t\mapsto M_t^{(0)}(\omega(t))$ and $t \mapsto M_t^{(1)}$ are  absolutely continuous on $t\in [0, T]$.
            \end{enumerate}
    \end{enumerate}
\end{hyp}
...
\end{equation}
has a unique solution, in the Lebesgue sense, for each realization $X_0 = X_0(\omega)$ of the initial condition and each sample path $t\mapsto Y_t(\omega)$ of the noise process, for $\omega\in \Omega$.
...
\[
    |X_t| \leq \left(|X_0| + \int_0^T M_t^{(0)}\;\mathrm{d}t\right) e^{\int_0^t M_t^{(1)}\;\mathrm{d}t}.
\]

Notice the absolute continuity of the sample paths of $\{M_t^{(0)}\}_{t\in I}$ and $\{M_t^{(1)}\}_{t\in I}$ are not needed for this bound, but are required for the existence of sample paths solutions in the Carath\'eodory theory (see \cite[Theorem 2.3]{HanKloeden2017}).

Moreover, the requirement that their sample paths be absolutely continuous is not a direct requirement on the regularity of the noise. They are just bounds. For instance, if the sample paths of the noise are just continuous, then, each sample path is uniformly bounded on the compact time interval $[0, T]$ and, hence, we can bound the sample path of the noise by the constant path given by the maximum $M(\omega) = \max_{t\in [0, T]}|Y_t(\omega)|$. If the noise is a renewal process, with sample paths with jump discontinuities, we can bound its sample paths by the piecewise linear paths joining the discontinuity values. And so on.

For the strong convergence of the Euler-Maruyama approximation, we also need to control the expectation of the solution above, among other things. One common situation is when $\mathbb{E}[|X_0|] < \infty$, $\mathbb{E}[|M_t^{(0)}|] < \infty$ and $\sup_\omega |M_t^{(1)}| < \infty$. In special dissipative cases, depending on the structure of the equation, that might amount to only requiring $\mathbb{E}[|X_0|] < \infty$. Due to the diverse ways in which to control the solution, we leave further hypotheses to the applications.
...
\begin{lem}
    ...
    \begin{equation}
        \label{estimateforI2}
        K_2 = \left(\int_0^{t_j} \mathbb{E}\left[\left|B_s \partial_y f(s, X_{\tau^N(s)}^N, Y_s)\right|^2\right] \;\mathrm{d}s\right) < \infty.
    \end{equation}
    Then,
    \begin{equation}
...
\begin{proof}
    Taking the expectation of the absolute value of \eqref{rewritenoiseterm}, we see we need to estimate $\mathbb{E}[|I^1|]$ and $\mathbb{E}[|I^2|]$, defined in Lemma \ref{lemrewritenoiseterm}. The estimate of the first term is straightfoward and leads to
    \begin{align*}
...
\begin{lem}
    Suppose $f=f(t, x, y)$ is uniformly globally Lipschitz continuous on $x$ with Lipschitz constant $L_x$, as in \eqref{Lxassumptionbasic} and that it is twice continuously differentiable on $(t, y)\in [0, T]\times \mathbb{R}$, for each $x\in \mathbb{R}$. Suppose, moreover, that all the conditions in Lemma \ref{estimatesecondterminglobalerror} hold and that \eqref{estimateforI1} and \eqref{estimateforI2} also hold. Then, the Euler-Maruyama scheme \eqref{emscheme}-\eqref{iccondition} is of strong order 1, i.e.
    \begin{equation}
...
The required conditions in Lemma \ref{lemestimatesforI1andI2} are not readily checked. We present, below, some particular cases with more explicit bounds that imply those.
