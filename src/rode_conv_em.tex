...
\]
But in order for this to be meaningful, we need to show that some reasonable/interesting examples satisfy these conditions, e.g. we should try $g(t, Y_t) = Y_t$ with some transport noise $Y_t = (\sin(\Omega t))^{1/3}$ or a process with Takagi-Landsberg sample paths with varing Hurst coefficients, or whatever.
