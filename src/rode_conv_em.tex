...
Our first example is the population dynamics model taken from \cite[Section 15.2]{HanKloeden2017}, given by
\begin{equation}
    \label{rodepopulationdynamics}
    \frac{\mathrm{d}X_t}{\mathrm{d}t} = \lambda X_t (r - X_t)\cos(Y_t)
\end{equation}
where $\lambda, r > 0$ are constant and $\{Y_t\}_{t\geq 0}$ is the Ornstein-Uhlenbeck process given by
\[
    \mathrm{d}Y_t = (\theta_1 - \theta_2 Y_t)\;\mathrm{d}t + \theta_3\;\mathrm{d}W_t,
\]
with $\theta_1, \theta_2, \theta_3 > 0$ constant and $\{W_t\}_{t\geq 0}$ being a standard Wiener process. We do not need to approximate the couple stochastic differential equation system for $(X_t, Y_t)$ since the Ornstein-Uhlenbeck process has an analytic solution, given by
\[
    Y_t = \frac{\theta_1}{\theta_2} + e^{-\theta_2 (t - s)}\left(Y_s - \frac{\theta_1}{\theta_2}\right) + \theta_3\int_s^t e^{-\theta_2 (t - \sigma)}\;\mathrm{d}W_\sigma.
\]
...
\[
    0 \leq X_0 \leq R,
\]
for some $R > r$. In this case, the pathwise solutions of \eqref{rodepopulationdynamics} are almost surely bounded as well, with
\[
    0 \leq X_t \leq R,
\]
for all $t \geq 0$.

The right hand side of \eqref{rodepopulationdynamics} is not globally Lipscthiz, but, since the solutions are bounded, we can consider, for the sake of analysis, the right hand side
\[
    f_R(t, x, y) = \begin{cases}
        \lambda x (r - x)\cos(y), & x \leq R, \\
        \lambda x \left(r - \frac{2Rx}{R + x}\right)\cos(y), & x > R,
    \end{cases}
\]
so the RODE \eqref{rodepopulationdynamics} with $0\leq X_0 \leq R$, $R > r$, is equivalent to the RODE
\begin{equation}
    \label{rodepopulationdynamicstruncated}
    \frac{\mathrm{d}X_t}{\mathrm{d}t} = f_R(t, X_t, Y_t).
\end{equation}

The function $f_R=f_R(t, x, y)$ is continuous on all variables; is uniformly globally Lipschitz continuous on $x$; and satisfies the bound
\[
    |f_R(t, x, y)| \leq \lambda(r + R) |x|.
\]
Hence, the \cref{standinghypotheses} hold. Moreover, it follows from \eqref{XtboundLXMt} (notice $M_t = 0$ here) that
\[
    |X_t| \leq |X_0|e^{\lambda(r + R)t} \leq R e^{\lambda(r + R)T}, \qquad 0 \leq t \leq T.
\]
almost surely. 
