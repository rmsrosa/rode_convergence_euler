    ...
    \begin{enumerate}
        ...
        \item We also assume that $(t, x) \mapsto f(t, x, Y_t)$ satisfies the Carath\'eodory conditions:
            \begin{enumerate}
                ...
                \item The bound $|f(t, x, Y_t)| \leq M_t + L_X|x|$ holds for all $t\in I$ and all $x\in\mathbb{R}$, where each sample path $t\mapsto M_t(\omega)$ is absolutely continuous on $t\in [0, T]$.
            \end{enumerate}
    \end{enumerate}
\end{hyp}
...
\[
    |X_t| \leq \left(|X_0| + \int_0^T M_t\;\mathrm{d}t\right) e^{L_X t}.
\]
...
\[
    \mathbb{E}[|X_0|] < \infty, \qquad \int_0^T \mathbb{E}[|M_t|] \;\mathrm{d}t < \infty,
\]
in which case
\[
    \mathbb{E}[|X_t|] \leq \left(\mathbb{E}[|X_0|] + \int_0^t \mathbb{E}[|M_t|]\;\mathrm{d}t\right) e^{L_X t}.
\]
