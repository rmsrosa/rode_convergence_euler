...
\begin{lem}
    ...
    \[
        |f(s, x, Y_s) - f(t, x, Y_t)| \leq (G_s - G_t) h(x) + G^0_s - G^0_t,
    \]
    where $\{G_t\}_{t\in I}$ and $\{G^0_t\}_{t\in I}$ are real random processes with monotonically increasing sample paths and $h$ is a nonnegative function. Then,
    \begin{multline*}
        \left|\int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right| \\
         \leq \left(\sup_{t\in [0, T]}\{|h(X_{\tau^N(s)})|\}(G_T - G_0) + G^0_T - G^0_0 \right)\Delta t. 
    \end{multline*}
\end{lem}
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        \leq (G_s - G_{\tau^N(s)})h(X_{\tau^N(s)}^N) + G^0_s - G^0_{\tau^N(s)}
    \end{multline*}
    
    Thus, we have
    \begin{multline*}
        \left|\int_0^{t_j} \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right| \\
         \leq \int_0^T \left((G_t - G_{\tau^N(t)})h(X_{\tau^N(t)}) + G^0_t - G^0_{\tau^N(t)}\right)\;\mathrm{d}t.
    \end{multline*}
    The first term in the integration is estimated as
    \begin{multline*}
        \int_0^T (G_t - G_{\tau^N(t)})h(X_{\tau^N(t)})\;\mathrm{d}t \\
        \leq \sup_{t\in [0, T]}\{|h(x(\tau^N(t))|\} \int_0^T (G_t - G_{\tau^N(t)})\;\mathrm{d}t
    \end{multline*}

    Now, the remaining integral we split at time $t_1 = \Delta t$ and use the monotonicity:
    \begin{align*}
        \int_0^T (G_t - G_{\tau^N(t)})\;\mathrm{d}t & = \int_0^T G_t \;\mathrm{d}t - \int_0^{t_1} G_{\tau^N(t)}\;\mathrm{d}t - \int_{t_1}^T G_{\tau^N(t)}\;\mathrm{d}t \\
        & \leq \int_0^T G_t \;\mathrm{d}t - \int_0^{\Delta t} G_0\;\mathrm{d}t - \int_{\Delta t}^T G_{t-\Delta t}\;\mathrm{d}t \\
        & = \int_0^T G_t \;\mathrm{d}t - \int_0^{\Delta t} G_0\;\mathrm{d}t - \int_{0}^{T-\Delta t} G_t\;\mathrm{d}t \\
        & = \int_{T-\Delta t}^T G_t \;\mathrm{d}t - G_0\Delta t \\
        & \leq (G_T - G_0)\Delta t.
    \end{align*}

    Similarly,
    \[
        \int_0^T (G^0_t - G^0_{\tau^N(t)})\;\mathrm{d}t \leq (G^0_T - G^0_0)\Delta t
    \]
...
\begin{thm}
    ...
    \[
        |f(s, x, Y_s) - f(t, x, Y_t)| \leq (G_s - G_t) h(x) + G^0_s - G^0_t,
    \]
    where $\{G_t\}_{t\in I}$ and $\{G^0_t\}_{t\in I}$ are real random processes with monotonically increasing sample paths and $h$ is a nonnegative function such that, for some $1\leq p, q \leq \infty$ with $1/p + 1/q = 1$, it holds
    \[
        \mathbb{E}\left[\left(\sup_{t\in [0, T]}h(X_t)\right)^p\right] < \infty, \quad \mathbb{E}[(G_T - G_0)^q] < \infty, \quad \mathbb{E}[G^0_T - G^0_0] < \infty,
    \]
...
\begin{rmk}
    ...
    $$
        f(t, x, Y_t) \leq C(1 + |x| + |Y_t|) \leq C(1 + |x| + G_t)
    $$
    where $G_t = \sup_{0 \leq s \leq t} |Y_t|$ is monotonically nondecreasing and we just need a bound
    $$
        \mathbb{E}[(\sup_{0\leq t \leq T} |Y_t|)^q] < \infty
    $$
\end{rmk}
