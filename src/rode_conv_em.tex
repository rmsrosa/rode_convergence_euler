...
\begin{lem}
    The Euler-Maruyama approximation \eqref{emscheme}-\eqref{iccondition} for the random ordinary differential equation \eqref{rodeeq} satisfies the global error formula
    \begin{equation}
...
\begin{proof}
First, we obtain an expression for a single time step, from time $t_{j-1}$ to $t_j = t_{j-1} + \Delta t$. For notational simplicity, we momentarily write $t = t_{j-1}$ and $\tau = \Delta t$, so that $t_j = t + \tau$. The exact solution satisfies
$$
...
Now we iterate the time steps \eqref{singlestep} to find, using the initial condition \eqref{iccondition} that $X_0^N = X_0$,
\begin{align*}
