...
The first term in the right hand side of \eqref{Etjbasicbound} usually vanishes since in general we take $X_0^N = X_0$, but it suffices to assume that $X_0^N$ approximates $X_0$ of order $\Delta t_N$, which is useful for lower order approximations or for the discretization of random partial differential equations.
...
\begin{proof}
    ...
    Taking the expectation of both sides of \eqref{estimatesecondterminglobalerrorintegral} and using \eqref{XtboundLXMt} and \eqref{X0EMsconditions} we find \eqref{expectedestimatesecondterminglobalerrorintegral}.
\end{proof}
...
\begin{prop}
    Under the \cref{standinghypotheses1}, suppose further that
    \begin{enumerate}
        \item the initial condition is strongly bounded, i.e.
        \begin{equation}
            \label{EX0condition}
            \mathbb{E}[|X_0|] < \infty;
        \end{equation}
        \item the non-negative process $\{M_t\}_{t\in I}$ is strongly integrable, i.e.
        \begin{equation}
            \label{EMcondition}
            \int_0^T \mathbb{E}[M_s] \;\mathrm{d}s < \infty;
        \end{equation}
        \item for some constant $C_0 \geq 0$, 
        \begin{equation}
            \label{EX0X0N}
...
\begin{proof}
    \cref{lembasicestimate} and \cref{estimatesecondterminglobalerror}.

    Applying \eqref{EX0condition} and \eqref{EMcondition} to the estimate \eqref{estimatesecondterminglobalerrorintegral} of \cref{estimatesecondterminglobalerror} yields
    \[
        \int_0^{t_j} \mathbb{E}[|X_s - X_{\tau^N(s)}]| \;\mathrm{d}s \leq \Delta t_N \int_0^{t_j} (\mathbb{E}[M_s] + L_X\mathbb{E}[|X_s|]) \;\mathrm{d}s.
    \]
    Using \eqref{XtboundLXMt}, we obtain
    \begin{align*}
    ...
    \end{equation}
    Taking the expectation of the global error formula \eqref{Etjbasicbound} gives us
    \begin{multline*}
    ...
    \end{multline*}
    Using estimate \eqref{expectedestimatesecondterminglobalerrorintegral} and condition \eqref{EX0X0N} proves \eqref{expectedestimateglobalerrorintegral}.
...
\begin{lem}
    ...
    \begin{equation}
        \label{expectmonotonicbound}
          \mathbb{E}\left[|f(s, X_\tau, Y_s) - f(\tau, X_\tau, Y_\tau)|\right] \leq e(s) - e(\tau),
      \end{equation}
      where $e = e(s)$ is a monotonically non-decreasing function in $s\in I$. Then,
      \begin{equation}
        \label{expectintfboundbyG}
          \mathbb{E}\left[\left|\int_0^t \left( f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)}) \right)\;\mathrm{d}s\right|\right] \leq (e(t) - e(0))\Delta t_N,
      \end{equation}
      for all $0 \leq t \leq T$ and every $N\in \mathbb{R}$.
\end{lem}

\begin{proof}
    Let $N\in \mathbb{R}$. From the assumption \eqref{expectmonotonicbound} we have
    \[
        \mathbb{E}\left[|f(s, X_{\tau^N(s)}^N, Y_s) - f(\tau^N(s), X_{\tau^N(s)}^N, Y_{\tau^N(s)})|\right] \leq e(s) - e(\tau^N(s)),
    \]
    ...
    \begin{align*}
        ...
        & \qquad \leq \int_0^t \left(e(s) - e(\tau^N(s))\right)\;\mathrm{d}s.
    \end{align*}
    Now we need to bound the right hand side. When $0 \leq t\leq t_1 = \Delta t_N$, we have $\tau^N(s) = 0$ for all $0 \leq s < t_1$, so that,
    \begin{align*}
      \int_0^t (e(s) - e(\tau^N(s)))\;\mathrm{d}s & = \int_0^t (e(s) - e(0)) \;\mathrm{d}s.
    \end{align*}
    Using the monotonicity,
    \[
      \int_0^t (e(s) - e(\tau^N(s)))\;\mathrm{d}s \leq \int_0^t (e(t) - e(0)) \;\mathrm{d}s  = (e(t) - e(0))t \leq (e(t) - e(0))\Delta t_N.
    \]
    When $\Delta t_N\leq t \leq T$, we split the integration of the second term at time $s = t_1 = \Delta t_N$ and write
    \[ 
      \int_0^t (e(s) - e(\tau^N(s)))\;\mathrm{d}s = \int_0^t e(s) \;\mathrm{d}s - \int_0^{t_1} e(\tau^N(s))\;\mathrm{d}s - \int_{t_1}^t e(\tau^N(s))\;\mathrm{d}s
    \]
    Using the monotonicity together with the fact that $s - \Delta t_N\leq \tau^N(s) \leq s$ for all $\Delta t_N\leq s \leq T$,
    \begin{align*}
        \int_0^t (e(s) - e(\tau^N(s)))\;\mathrm{d}s & \leq \int_0^t e(s) \;\mathrm{d}s - \int_0^{\Delta t_N} e(0)\;\mathrm{d}s - \int_{\Delta t_N}^t e(s-\Delta t_N)\;\mathrm{d}s \\
        & = \int_0^t e(s) \;\mathrm{d}s - \int_0^{\Delta t_N} e(0)\;\mathrm{d}s - \int_{0}^{T-\Delta t_N} e(s)\;\mathrm{d}s \\
        & = \int_{t-\Delta t_N}^t e(s) \;\mathrm{d}s - e(0)\Delta t_N.
    \end{align*}
    Using again the monotonicity yields
    \[ 
      \int_0^t (e(s) - e(\tau^N(s)))\;\mathrm{d}s \leq \int_{t-\Delta t_N}^t e(t) \;\mathrm{d}s - e(0)\Delta t_N= (e(t) - e(0))\Delta t_N.
    \]
...
\begin{thm}
    ...
    Under the \cref{standinghypotheses2}, suppose, moreover, that \eqref{X0EMsconditions}, \eqref{estimateforI1} and \eqref{estimateforI2} hold. Then, the Euler-Maruyama scheme \eqref{emscheme}-\eqref{iccondition} is of strong order 1, i.e.
    \begin{equation}
