...
Our first example is the population dynamics model modified from \cite[Section 15.2]{HanKloeden2017},
\begin{equation}
    \label{rodepopulationdynamics}
    \frac{\mathrm{d}X_t}{\mathrm{d}t} = \Lambda_t X_t (r - X_t)
\end{equation}
where $r > 0$ is constant and the growth parameter $\{\Lambda_t\}_{t \geq}$ is a stochastic process given by
\[
    \Lambda_t = \lambda (1 + \varepsilon \sin(O_t)),
\]
...
\[
    0 \leq X_t \leq R,
\]
for all $t \geq 0$. The noise process $\{\Lambda_t\}_{t \geq 0}$ itself satisfies
\[
    0 < \lambda - \varepsilon \leq \Lambda_t \leq \lambda + \varepsilon, \qquad \forall t \geq 0.
\]

The right hand side of \eqref{rodepopulationdynamics} is not globally Lipschitz, but, since the solutions are bounded, we can consider, for the sake of analysis, the right hand side
\[
    f_R(t, x, \lambda) = \begin{cases}
        \lambda x (r - x), & x \leq R, \\
        \lambda x \left(r - \frac{2Rx}{R + x}\right), & x > R,
    \end{cases}
\]
...
\begin{equation}
    ...
    \frac{\mathrm{d}X_t}{\mathrm{d}t} = f_R(t, X_t, \Lambda_t).
\end{equation}
