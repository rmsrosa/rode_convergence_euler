...
In the case of an It\^o noise, the sample paths are typically H\"older continuous with exponent 1/2, but the It\^o isometry reveals futher regularity that can be used to prove a strong order 1 convergence. In the case of transport process $Y_t = g(t, Y_0)$, for a random vector $Y_0 = Y_0(\omega)$, even if $g=g(t, y)$ is H\"older continuous, many common H\"older functions are actually of bounded variation, such as $g(t, y) = (yt)^{1/3}$ or even $g(t, y) = \sin(yt)^{1/3}$. The same with point-processes $Y_t = ...$, which are of bounded variation.
...
\begin{proof}
  ...
  \end{align*}
  for a suitable $K$. This proves \eqref{thmmonotonicboundstrongorder} with the constant $C$ given by \eqref{thmmonotonicboundconstant}.
\end{proof}
