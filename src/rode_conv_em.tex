...
\begin{lem}
    ...
    \end{equation}
    where $\{G_t\}_{t\in I}$ is a real random processes with monotonically non-decreasing sample paths. Then,
    \begin{equation}
      \label{intfboundbyG}
...
\begin{proof}
    ...
    \]
    Now we need to bound the right hand side. When $0 \leq t\leq t_1 = \Delta t$, we have $\tau^N(s) = 0$ for all $0 \leq s < t_1$, so that,
    \begin{align*}
    ...
    \]
    When $\Delta t \leq t \leq T$, we split the integration of the second term at time $s = t_1 = \Delta t$ and write
    \[ 
      \int_0^t (G_s - G_{\tau^N(s)})\;\mathrm{d}s = \int_0^t G_s \;\mathrm{d}s - \int_0^{t_1} G_{\tau^N(s)}\;\mathrm{d}s - \int_{t_1}^t G_{\tau^N(s)}\;\mathrm{d}s
    \]
    Using the monotonicity together with the fact that $s - \Delta t \leq \tau^N(s) \leq s$ for all $\Delta t \leq s \leq T$,
    \begin{align*}
    ...
    \[ 
      \int_0^t (G_s - G_{\tau^N(s)})\;\mathrm{d}s \leq \int_{t-\Delta t}^t G_t \;\mathrm{d}s - G_0\Delta t = (G_t - G_0)\Delta t.
    \]
...
\begin{thm}
    Suppose that $f=f(t, x, y)$ is continuous on $[0, T]\times \mathbb{R}\times\mathbb{R}$ and is globally Lipschitz continuous in $x$, uniformly in $t$ and $y$, and that $\sup_{t\in [0, T]}\mathbb{E}[|X_t|] < \infty$. Suppose, further, that, for all $0 \leq \tau \leq s \leq T$,
    \begin{equation}
      \label{thmmonotonicbound}
        |f(s, X_\tau, Y_s) - f(\tau, X_\tau, Y_\tau)| \leq G_s - G_\tau,
    \end{equation}
    where $\{G_t\}_{t\in I}$ is a real random processes with monotonically non-decreasing sample paths and such that
    \[
