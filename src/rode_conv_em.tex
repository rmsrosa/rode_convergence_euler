...
\begin{proof}
    ...
    Taking the absolute value and using that the first order derivatives of $a=a(t, y)$ and $b=b(t, y)$ are uniformly bounded we find that
    \begin{align*}
        |f(s, X_\tau, Y_s) - f(\tau, X_\tau, Y_\tau)| & \leq \int_\tau^s \left(A_T|h(X_\tau)| + B_T\right) \;\mathrm{d}\xi \\
        & \qquad + \int_\tau^s \left(A_Y |h(X_\tau)| + B_Y\right)\;|\mathrm{d}Y_\xi|.
    \end{align*}
    Define the process $\{G_t\}_{t\in I}$ by
    \[
        G_t = \int_0^t \left(A_T|h(X_{\tau(\xi)})| + B_T\right) \;\mathrm{d}\xi + \int_0^t \left(A_Y |h(X_{\tau(\xi)})| + B_Y\right)\;|\mathrm{d}Y_\xi|.
    \]
\end{proof}
