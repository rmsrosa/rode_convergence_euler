...
\begin{proof}
  ...
  For the first integral term, since $s - \tau^N(s) \leq \Delta t$ and using the control on $s \mapsto f(s, X_s, Y_s)$, we observe that
  \begin{align*}
...
One typical case in which a bound such as that in Theorem \ref{thmmonotonicbound} is possible is that of linear equation. More generally, we may assume $f$ is separable in a certain sense:
