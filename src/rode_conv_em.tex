...
One typical case in which a bound such as that in \cref{thmmonotonicbound} is possible is that of a linear equation, or, when $f$ is \emph{semi-separable}, in the sense of being of the form $f(t, x, y) = a(t, y)h(t, x) + b(t, y)$ with suitable functions $a=a(t, y), b=b(t, y)$, and $h=h(t, x)$. More precisely, we have the following result.
