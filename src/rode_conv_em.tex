...
As for the second term, that just concerns the solution itself, not the approximation, and for that we have the following result.
...
\begin{lem}
    ...
    \begin{equation}
      |f(s, X_s, Y_s)| \leq F_t,
    \end{equation}
    where $\{F_t\}_{t\in I}$ is a real random process satisfying
    \begin{equation}
...
We need to estimate the global error \eqref{Etjbasicbound}.
