...
\newtheorem{stdhyp}{Assumptions}[section]
...
Under these assumptions, for each sample $\omega\in\Omega$, the integral equation
\begin{equation}
...
When $f=f(t, x, y)$ is continuous on all three variables, as well as uniformly globally Lipscthiz continuous in $x$, and the sample paths of $\{Y_t\}_{t\geq 0}$ are continuous, then the integrand in \eqref{integralrodeform} is continuous in $t$ and the integral becomes a Riemann integral. In this case, the integral form \eqref{integralrodeform} of the pathwise solutions of \eqref{rodeeq} holds in the Riemann sense and the estimate \eqref{XtboundLXMt} is still valid.
...
\begin{lem}
    ...
    If, moreover,
    \begin{equation}
        \label{X0EMsconditions}
        \mathbb{E}[|X_0|] < \infty, \quad \int_0^T \mathbb{E}[M_s] \;\mathrm{d}s < \infty
    \end{equation}
    then
    \begin{equation}
    ...
    \begin{equation}
      \label{Kconstantinestimatesecondterminglobalerror}
        K = \left(\mathbb{E}[|X_0|] + \int_0^T \mathbb{E}[|M_\xi|]\;\mathrm{d}\xi\right)e^{L_X T}.
    \end{equation}
\end{lem}
