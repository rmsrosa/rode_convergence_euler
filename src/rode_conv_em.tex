...
\begin{proof}
    ...
    \]
    Since the sample paths of $\{Y_t\}_{t\in I}$ are of bounded variation, the process $\{M_t\}_{t\in I}$ has integrable sample paths. This means that we are under the \cref{standinghypotheses1}.
    
    Thanks to \eqref{EYtboundedsquarevariation2}, we see that
    \[
        \mathbb{E}[|Y_t|^2] \leq \mathbb{E}[V(\{Y_t\}_{t\in I}; I)^2] < \infty
    \]
    so that $\{M_t\}_{t\in I}$ satisfies \eqref{EMtstrongbound}. Moreover, by assumption, \eqref{EX0strongbound} also holds, so that, from \eqref{XtboundLXMt}, we have
    \[
