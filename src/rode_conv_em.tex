...
\begin{equation}
  ...
  \end{cases}
\end{equation}
where $\{Y_t\}_{t\in I}$ is a real stochastic process with continuous sample paths on the time interval $I=[0, T]$; $f:I \times \mathbb{R} \times\mathbb{R} \rightarrow \mathbb{R}$ is continuous; and $X_0$ is a real random variable. The sample space is denoted by $\Omega$. A similar result holds for systems of random ordinary equations, as discussed later in the article, but we start with the scalar case to keep the ideas simple and clear.
